...
buildScripts="$( realpath ${scriptsDir}/build_with_environment.sh )"
environmentsDir="$( realpath ${scriptsDir}/../TestEnvironments )"

echo "Dir: ${environmentsDir}"
...
do
  ...
  source $buildScripts --gemfile="${environmentDir}/Gemfile" --podfile="${environmentDir}/Podfile"
  echo "--- Build for environment from ${environmentDir} finished ---"
done
