...
// TODO: This is only used for tests.  We should fix them to go through
// FIRDatabase and remove this method and the sessionsConfigs dictionary
// (FIRDatabase automatically creates one config per app).
+ (FIRDatabaseConfig *)configForName:(NSString *)name {
    ...
    if ([expression numberOfMatchesInString:name
        ...
        [NSException raise:NSInvalidArgumentException
                    format:@"Name can only contain [a-zA-Z0-9-_]"];
    }
    ...
    @synchronized(sessionConfigs) {
        if (!sessionConfigs[name]) {
            id<FAuthTokenProvider> authTokenProvider = [FAuthTokenProvider
                authTokenProviderForApp:[FIRApp defaultApp]];
            sessionConfigs[name] = [[FIRDatabaseConfig alloc]
                initWithSessionIdentifier:name
