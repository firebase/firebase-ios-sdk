    ...
    if (![self isValidPathString:pathString]) {
        @throw [[NSException alloc]
            initWithName:@"InvalidPathValidation"
                  reason:[NSString stringWithFormat:@"(%@) Must be a non-empty "
                                                    @"string and not contain "
                                                    @"'.' '#' '$' '[' or ']'",
                                                    fn]
                userInfo:nil];
    }
    ...
    if (![self isValidKey:key]) {
        @throw [[NSException alloc]
            initWithName:@"InvalidKeyValidation"
                  reason:[NSString
                             stringWithFormat:@"(%@) Must be a non-empty "
                                              @"string and not contain '/' '.' "
                                              @"'#' '$' '[' or ']'",
                                              fn]
                userInfo:nil];
...
+ (BOOL)validateFrom:(NSString *)fn
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        ...
        if (dval[kServerValueSubKey] != nil) {
            ...
            if (throwError) {
                ...
                @throw [[NSException alloc]
                    initWithName:@"InvalidFirebaseData"
                          reason:[NSString
                                     stringWithFormat:@"(%@) Cannot store an "
                                                      @"NSDictionary as "
                                                      @"priority at path: %@.",
                                                      fn, pathString]
                        userInfo:nil];
        ...
        if (throwError) {
            ...
            @throw [[NSException alloc]
                initWithName:@"InvalidFirebaseData"
                      reason:[NSString stringWithFormat:@"(%@) Cannot store an "
                                                        @"NSArray as priority "
                                                        @"at path: %@.",
                                                        fn, pathString]
                    userInfo:nil];
