...
- (NSComparisonResult)compareKey:(NSString *)key1
    ...
    if (reverse) {
        return
            [self compareKey:key2 andNode:node2 toOtherKey:key1 andNode:node1];
    } else {
        return
            [self compareKey:key1 andNode:node1 toOtherKey:key2 andNode:node2];
    }
}
...
- (FNamedNode *)maxPost {
    ...
    return
        [[FNamedNode alloc] initWithName:[FUtilities maxName] andNode:maxNode];
}

- (FNamedNode *)makePost:(id<FNode>)indexValue name:(NSString *)name {
    id<FNode> node =
        [[FEmptyNode emptyNode] updateChild:self.path withNewChild:indexValue];
