...
+ (FCompoundWrite *)compoundWriteWithValueDictionary:
    ...
    [dictionary enumerateKeysAndObjectsUsingBlock:^(NSString *pathString,
      ...
      writeTree =
          [writeTree setTree:tree atPath:[[FPath alloc] initWith:pathString]];
    }];
...
- (FCompoundWrite *)addWrite:(id<FNode>)node atPath:(FPath *)path {
    if (path.isEmpty) {
        ...
        if (rootMost != nil) {
            FPath *relativePath =
                [FPath relativePathFrom:rootMost.path to:path];
            id<FNode> value =
                [rootMost.value updateChild:relativePath withNewChild:node];
            return [[FCompoundWrite alloc]
            ...
            FImmutableTree *newWriteTree =
                [self.writeTree setTree:subtree atPath:path];
...
- (NSArray *)completeChildren {
    ...
    if (self.writeTree.value != nil) {
        ...
        [node enumerateChildrenUsingBlock:^(NSString *key, id<FNode> node,
                                            BOOL *stop) {
          [children
              addObject:[[FNamedNode alloc] initWithName:key andNode:node]];
        }];
    } else {
