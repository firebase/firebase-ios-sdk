...
- (void)addMergeAtPath:(FPath *)path
    ...
    self.visibleWrites =
        [self.visibleWrites addCompoundWrite:changedChildren atPath:path];
    self.lastWriteId = writeId;
}
...
- (BOOL)removeWriteId:(NSInteger)writeId {
    ...
    while (removedWriteWasVisible && i >= 0) {
        ...
        if (currentWrite.visible) {
            if (i >= index &&
                [self record:currentWrite containsPath:writeToRemove.path]) {
    ...
    if (topLevelSet != nil) {
        if (![topLevelSet isLeafNode]) {
            ...
            [topChildrenNode enumerateChildrenUsingBlock:^(
              ...
              completeChildren =
                  [completeChildren updateImmediateChild:key withNewChild:node];
            }];
        }
      ...
      if (filter(record)) {
          ...
          if ([record isOverwrite]) {
              if ([treeRoot contains:writePath]) {
                  FPath *relativePath =
                      [FPath relativePathFrom:treeRoot to:writePath];
                  ...
                  compoundWrite =
                      [compoundWrite addWrite:child atPath:[FPath empty]];
              } else {
              ...
              if ([treeRoot contains:writePath]) {
                  ...
                  FPath *relativePath =
                      [FPath relativePathFrom:writePath to:treeRoot];
