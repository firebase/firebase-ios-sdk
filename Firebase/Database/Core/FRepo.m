...
- (void)set:(FPath *)path
    ...
    FPath *affectedPath =
        [self abortTransactionsAtPath:path error:kFTransactionSet];
...
- (void)updateInfo:(NSString *)pathString withValue:(id)value {
    ...
    NSArray *events =
        [self.infoSyncTree applyServerOverwriteAtPath:path newData:newNode];
    [self.eventRaiser raiseEvents:events];
}
...
- (void)onDataUpdate:(FPersistentConnection *)fpconnection
    ...
    if (tagId != nil) {
        ...
        events =
            [self.serverSyncTree applyServerOverwriteAtPath:path newData:snap];
    }
...
// release build.
#if TARGET_OS_IOS
    // The idea is to wait until any outstanding sets get written to disk.
    // Since the sets might still be in our dispatch queue, we wait for the
    // dispatch queue to catch up and for persistence to catch up. This may be
...
- (void)startTransactionOnPath:(FPath *)path
    ...
    if (self.config.persistenceEnabled &&
        ...
        FFInfo(@"I-RDB038020", @"runTransactionBlock: usage detected while "
                               @"persistence is enabled. Please be aware that "
                               @"transactions "
                               @"*will not* be persisted across app restarts. "
                               @"See "
                               @"https://www.firebase.com/docs/ios/guide/"
                               @"offline-capabilities.html#section-handling-"
                               @"transactions-offline for more details.");
    }
...
- (void)sendTransactionQueue:(NSMutableArray *)queue atPath:(FPath *)path {
    ...
    id<FNode> latestState =
        [self latestStateAtPath:path excludeWriteIds:writeIdsToExclude];
    ...
    if (queue != nil) {
        ...
        for (FTupleTransaction *transaction in queue) {
            if (transaction.status == FTransactionSentNeedsAbort) {
                ...
                if (transaction.onComplete) {
                    NSError *abortReason =
                        [FUtilities errorForStatus:error andReason:nil];
