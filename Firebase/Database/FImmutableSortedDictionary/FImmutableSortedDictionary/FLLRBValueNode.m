...
- (BOOL)inorderTraversal:(BOOL (^)(id key, id value))action {
    return [self.left inorderTraversal:action] ||
           action(self.key, self.value) || [self.right inorderTraversal:action];
}

- (BOOL)reverseTraversal:(BOOL (^)(id key, id value))action {
    return [self.right reverseTraversal:action] ||
           action(self.key, self.value) || [self.left reverseTraversal:action];
}
    ...
    if (blackDepth != [self.right check]) {
        NSString *err = [NSString
            stringWithFormat:@"(%@ -> %@)blackDepth: %d ; self.right check: %d",
                             self.value,
                             [self.color boolValue] ? @"red" : @"black",
                             blackDepth, [self.right check]];
        //        return 10;
        @throw
            [[NSException alloc] initWithName:@"check" reason:err userInfo:nil];
    } else {
        int ret = blackDepth + ([self isRed] ? 0 : 1);
        //        NSLog(@"black depth is: %d; other is: %d, ret is: %d",
        //        blackDepth, ([self isRed] ? 0 : 1), ret);
        return ret;
    }
}
