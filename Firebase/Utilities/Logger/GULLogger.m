...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"  // asl is deprecated
    ...
    BOOL debugMode = [userDefaults boolForKey:kGULPersistedDebugModeKey];

    if (disableDebug) {  // Default mode
      [userDefaults removeObjectForKey:kGULPersistedDebugModeKey];
    } else if (enableDebug || debugMode) {  // Debug mode
      [userDefaults setBool:YES forKey:kGULPersistedDebugModeKey];
...
void GULResetLogger() {
  ...
  [[NSUserDefaults standardUserDefaults] removeObjectForKey:kGULPersistedDebugModeKey];
}
