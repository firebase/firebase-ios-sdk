...
/** @property tenantID
    @brief The tenant ID indicating the specific OAuth provider this OAuthProvider instance
          represents.
 */
...
+ (instancetype)providerWithProviderID:(NSString *)providerID {
  return [self providerWithProviderID:providerID auth:[FIRAuth auth]];
}

+ (instancetype)providerWithProviderID:(NSString *)providerID auth:(FIRAuth *)auth {
  return [self providerWithProviderID:providerID auth:auth tenantID:auth.tenantID];
}

+ (instancetype)providerWithProviderID:(NSString *)providerID
                                  auth:(FIRAuth *)auth
                              tenantID:(NSString *)tenantID {
  return [[self alloc] initWithProviderID:providerID auth:auth tenantID:tenantID];
}
...
/** @fn initWithProviderID:auth:
    ...
    @param tenantID The tenant ID of the IDP for which this auth provider instance will be
        configured.
...
- (void)getHeadFulLiteURLWithEventID:(NSString *)eventID
  ...
  [FIRAuthWebUtils fetchAuthDomainWithRequestConfiguration:_auth.requestConfiguration
    ...
    NSString *tenantID = strongSelf.tenantID;
    if (tenantID) {
