...
extension GeminiDataModels {
  ...
  package struct GenerationConfig: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - candidateCount: Optional. Number of generated responses to return. If unset, this will default (behavior varies by backend). For more details, see ``candidateCount``.
    ///   - stopSequences: Optional. The set of character sequences (up to 5) that will stop output generation. (behavior varies by backend). For more details, see ``stopSequences``.
    ///   - maxOutputTokens: Optional. The maximum number of tokens to include in a response candidate. (behavior varies by backend). For more details, see ``maxOutputTokens``.
    ///   - temperature: Optional. Controls the randomness of the output. (behavior varies by backend). For more details, see ``temperature``.
    ///   - topP: Optional. The maximum cumulative probability of tokens to consider when sampling. (behavior varies by backend). For more details, see ``topP``.
    ///   - topK: Optional. The maximum number of tokens to consider when sampling. (behavior varies by backend). For more details, see ``topK``.
    ...
    ///   - logprobs: Optional. Only valid if response_logprobs=True. (behavior varies by backend). For more details, see ``logprobs``.
    ///   - enableEnhancedCivicAnswers: Optional. Enables enhanced civic answers. It may not be available for all models. (Gemini Developer API only). For more details, see ``enableEnhancedCivicAnswers``.
    ///   - responseModalities: Optional. The requested modalities of the response. Represents the set of modalities (behavior varies by backend). For more details, see ``responseModalities``.
    ///   - speechConfig: Optional. The speech generation config.
    ///   - thinkingConfig: Optional. Config for thinking features. (behavior varies by backend). For more details, see ``thinkingConfig``.
    ///   - imageConfig: Optional. Config for image generation. (behavior varies by backend). For more details, see ``imageConfig``.
