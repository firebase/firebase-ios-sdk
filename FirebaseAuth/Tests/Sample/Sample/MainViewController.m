...
- (BOOL)handleIncomingLinkWithURL:(NSURL *)URL {
    ...
    [self
        showTextInputPromptWithMessage:@"New Password:"
                       completionBlock:^(BOOL userPressedOK, NSString *_Nullable newPassword) {
                         ...
                         [self showSpinner:^() {
                           [[AppManager auth]
                               confirmPasswordResetWithCode:actionCode
                                                newPassword:newPassword
                                                 completion:^(NSError *_Nullable error) {
                                                   [self hideSpinner:^{
                                                     if (error) {
                                                       [self logFailure:
                                                                 @"Password reset in app failed"
                                                                  error:error];
                                                     ...
                                                     [self logSuccess:
                                                               @"Password reset in app succeeded."];
                                                     [self showMessagePrompt:
                                                               @"Password reset in app succeeded."];
                                                   }];
                                                 }];
  ...
  if ([mode isEqualToString:kVerifyEmailAction]) {
    [self
        showMessagePromptWithTitle:@"Tap OK to verify email"
                           message:actionCode
                            ...
                            [[AppManager auth]
                                applyActionCode:actionCode
                                     completion:^(NSError *_Nullable error) {
                                       [self hideSpinner:^{
                                         if (error) {
                                           [self logFailure:@"Verify email in app failed"
                                                      error:error];
...
- (void)updateTable {
  ...
  _tableViewManager.contents = [StaticContentTableViewContent contentWithSections:@[
    ...
    [StaticContentTableViewSection
        sectionWithTitle:kSectionTitleUserDetails
                   cells:@[
                     [StaticContentTableViewCell cellWithCustomCell:_userInfoTableViewCell
                                                             action:^{
                                                               [weakSelf presentUserInfo];
                                                             }],
                     [StaticContentTableViewCell cellWithCustomCell:_userToUseCell],
                     [StaticContentTableViewCell cellWithCustomCell:_userInMemoryInfoTableViewCell
                                                             action:^{
    ...
    // Settings
    [StaticContentTableViewSection
        sectionWithTitle:kSectionTitleSettings
                   cells:@[
                     ...
                     [StaticContentTableViewCell
                         cellWithTitle:kNewOrExistingUserToggleTitle
  ...
  [provider
      getAuthCredentialWithPresentingViewController:self
                                           callback:^(FIRAuthCredential *credential,
                                                      NSError *error) {
                                             if (!credential) {
                                               [self logFailedTest:@"The test needs a valid "
...
- (void)reauthenticate:(id<AuthProvider>)authProvider retrieveData:(BOOL)retrieveData {
  ...
  if (!user) {
    ...
    NSString *message = [NSString stringWithFormat:@"There is no signed-in %@ user.", provider];
                                               ...
                                               FIRAuthDataResultCallback completion = ^(
                                                 ...
                                                 if (error) {
                                                   if (error.code ==
                                                     ...
                                                     [self logFailure:
                                                               @"reauthenticate operation failed."
                                                                error:error];
                                                   }
                                                 } else {
                                                   [self
                                                       logSuccess:
                                                           @"reauthenticate operation succeeded."];
                                                 }
                                               ...
                                               FIRAuthDataResultCallback completion = ^(
                                                 ...
                                                 if (error) {
                                                   if (error.code ==
                                                     ...
                                                     [self logFailure:@"link auth provider failed"
                                                                error:error];
                                                   }
                                                 } else {
                                                   [self
                                                       logSuccess:@"link auth provider succeeded."];
                                                 }
                                                 if (authResult.additionalUserInfo) {
                                                   [self
                                                       logSuccess:[self
...
- (void)unlinkFromProvider:(NSString *)provider
  ...
  [[self user] unlinkFromProvider:provider
                       completion:^(FIRUser *_Nullable user, NSError *_Nullable error) {
                         ...
                         [self showTypicalUIForUserUpdateResultsWithTitle:@"Unlink from Provider"
                                                                    error:error];
                       }];
}
...
- (void)updateToSavedUser {
  ...
  if (!_userInMemory) {
    [self showMessagePrompt:
              [NSString stringWithFormat:@"You need an in-memory user to perform this"
                                          "action, use the M+ button to save a user to memory.",
                                         nil]];
    return;
  }

  [[AppManager auth]
      updateCurrentUser:_userInMemory
             completion:^(NSError *_Nullable error) {
               if (error) {
                 [self showMessagePrompt:[NSString stringWithFormat:@"An error Occurred: %@",
                                                                    error.localizedDescription]];
                 return;
               }
             }];
                  ...
                  showCancelButton:NO
                        completion:^(BOOL userPressedOK, NSString *_Nullable userInput) {
                          [self showMessagePromptWithTitle:@"Profile Info"
                                                   message:[self stringWithAdditionalUserInfo:
...
- (void)authStateChangedForAuth:(NSNotification *)notification {
  ...
  if (notification) {
    [self
        log:[NSString stringWithFormat:@"received FIRAuthStateDidChange notification on user '%@'.",
