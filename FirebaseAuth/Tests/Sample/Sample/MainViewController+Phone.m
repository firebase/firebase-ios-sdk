      ...
      sectionWithTitle:@"Phone Auth"
                 cells:@[
                   ...
                   [StaticContentTableViewCell
                       cellWithTitle:@"Unlink Phone Number"
                              action:^{
                                [weakSelf unlinkFromProvider:FIRPhoneAuthProvider.id
                                                  completion:nil];
                              }],
                 ]];
}
                 ...
                 [self hideSpinner:^{
                   ...
                   [self commonPhoneNumberInputWithTitle:@"Code"
                                              completion:^(NSString *_Nullable verificationCode) {
                                                [self
                                                    commontPhoneVerificationWithVerificationID:
                                                        verificationID
                                                                              verificationCode:
                                                                                  verificationCode];
                                                if (completion) {
                    ...
                    [self hideSpinner:^{
                      ...
                      if (self.isNewUserToggleOn) {
                        NSString *newUserString =
                            result.additionalUserInfo.isNewUser ? @"New user" : @"Existing user";
            ...
            verifyPhoneNumber:phoneNumber
                   ...
                   completion:^(NSString *_Nullable verificationID, NSError *_Nullable error) {
                     [self hideSpinner:
                               ^{
                                 if (error) {
                                   [self logFailure:@"failed to send verification code"
                                              error:error];
                                   [self showMessagePrompt:error.localizedDescription];
                                   if (completion) {
                                     completion(error);
                                                            ...
                                                            [self showSpinner:
                                                                      ^{
                                                                        FIRPhoneAuthCredential *
                                                                            credential = [[AppManager
                                                                                phoneAuthProvider]
                                                                                credentialWithVerificationID:
                                                                                    verificationID
                                                                                            verificationCode:
                                                                                                verificationCode];
                                                                        [[self user] linkWithCredential:
                                                                                         credential
                                                                                             completion:
                                                                                                 ^(FIRAuthDataResult
                                                                                                       *_Nullable result,
                                                                                                             ...
                                                                                                             ^{
                                                                                                               if (error) {
                                                                                                                 if (error
                                                                                                                   ...
                                                                                                                   [self
                                                                                                                       showMessagePromptWithTitle:
                                                                                                                           @"Phone number is already linked to "
                                                                                                                           @"another user"
                                                                                                                                          message:
                                                                                                                                              @"Tap Ok to sign in with that user now."
                                                                                                                                 showCancelButton:
                                                                                                                                     YES
                                                                                                                                       completion:^(
                                                                                                                                           BOOL
