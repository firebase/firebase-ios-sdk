                                                                                              ...
                                                                                              ^{
                                                                                                if (error) {
                                                                                                  if (error
                                                                                                    ...
                                                                                                    [self
                                                                                                        logFailure:
                                                                                                            @"sign-in with Email/Password failed"
                                                                                                             error:
                                                                                                                 error];
                                                                                                  }
                                                                                                } else {
                                                                                                  [self
                                                                                                      logSuccess:
                                                                                                          @"sign-in with Email/Password succeeded."];
                                                                                                  [self
                                                                                                      log:[NSString
                                                                                                              stringWithFormat:
                                                                                                                  @"UID: %@",
      ...
      [[self user]
          linkWithCredential:credential
                  completion:^(FIRAuthDataResult *result, NSError *error) {
                    ...
                    [self hideSpinner:^{
                      [self showTypicalUIForUserUpdateResultsWithTitle:@"Link with Email Password"
                                                                 error:error];
                    }];
                  }];
    }];
      ...
      [[self user]
          reauthenticateWithCredential:credential
                            completion:^(FIRAuthDataResult *_Nullable result,
                                         NSError *_Nullable error) {
                              if (error) {
                                [self logFailure:@"reauthicate with email password failed."
                                           error:error];
                              } else {
  ...
  [self
      showTextInputPromptWithMessage:@"Email Address:"
                     ...
                     completionBlock:^(BOOL userPressedOK, NSString *_Nullable email) {
                       ...
                       [self
                           showTextInputPromptWithMessage:@"Email Sign-In Link:"
                                          completionBlock:^(BOOL userPressedOK,
                                                            NSString *_Nullable link) {
                                            if (!userPressedOK) {
                                              return;
                                            }
...
- (void)sendEmailSignInLink {
  [self showTextInputPromptWithMessage:@"Email:"
                       completionBlock:^(BOOL userPressedOK, NSString *_Nullable userInput) {
                         ...
                         [self showSpinner:^{
                           void (^requestEmailSignInLink)(void (^)(NSError *)) =
      ...
      [[self user]
          linkWithCredential:credential
                  completion:^(FIRAuthDataResult *result, NSError *error) {
                    ...
                    [self hideSpinner:^{
                      [self showTypicalUIForUserUpdateResultsWithTitle:@"Link with Email Link"
                                                                 error:error];
                    }];
                  }];
    }];
