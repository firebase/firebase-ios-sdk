...
@interface MainViewController () <ASAuthorizationControllerDelegate,
                ...
                completion:^(FIRAuthDataResult *_Nullable authResult, NSError *_Nullable error) {
                  [self hideSpinner:^{
                    if (error) {
                      ...
                      [self logSuccess:[self stringWithAdditionalUserInfo:authResult
                                                                              .additionalUserInfo]];
                      if (self.isNewUserToggleOn) {
                        NSString *newUserString =
                            authResult.additionalUserInfo.newUser ? @"New user" : @"Existing user";
              ...
              completion:^(FIRAuthDataResult *_Nullable authResult, NSError *_Nullable error) {
                [self hideSpinner:^{
                  if (error) {
                    ...
                    [self
                        logSuccess:[self
                                       stringWithAdditionalUserInfo:authResult.additionalUserInfo]];
                    if (self.isNewUserToggleOn) {
                      NSString *newUserString =
                          authResult.additionalUserInfo.newUser ? @"New user" : @"Existing user";
                      [self showMessagePromptWithTitle:@"New or Existing"
                        ...
                        completion:^(FIRAuthDataResult *_Nullable authResult,
                          ...
                          [self hideSpinner:^{
                            if (error) {
                              ...
                              [self logSuccess:[self stringWithAdditionalUserInfo:
                            ...
                            [self showTypicalUIForUserUpdateResultsWithTitle:@"Reauthenticate Error"
                                                                       error:error];
                          }];
                        }];
  }];
                                         ...
                                         completion:^(FIRAuthDataResult *_Nullable authResult,
                                           ...
                                           [self hideSpinner:^{
                                             if (error) {
                                               [self
                                                   logFailure:
                                                       @"sign-in with Twitter (headful-lite) failed"
                                                        error:error];
                                               return;
                                             } else {
                                               [self logSuccess:@"sign-in with Twitter "
                                                                @"(headful-lite) succeeded."];
                                             }
                                             [self
  ...
  [self showTextInputPromptWithMessage:@"GitHub Access Token:"
                       completionBlock:^(BOOL userPressedOK, NSString *_Nullable accessToken) {
                         ...
                         FIROAuthCredential *credential =
                             [FIROAuthProvider credentialWithProviderID:FIRGitHubAuthProviderID
                           ...
                           [[AppManager auth]
                               signInWithCredential:credential
                                         completion:^(FIRAuthDataResult *_Nullable result,
                                                      NSError *_Nullable error) {
                                           if (error) {
                                             [self logFailure:@"sign-in with provider failed"
                                                        error:error];
                                           } else {
                                             [self logSuccess:@"sign-in with provider succeeded."];
                                           }
                                           [self
                                               showTypicalUIForUserUpdateResultsWithTitle:@"Sign-In"
                                                                                    error:error];
                                         }];
                         }
                       }];
                                         ...
                                         completion:^(FIRAuthDataResult *_Nullable authResult,
                                           ...
                                           [self hideSpinner:^{
                                             if (error) {
                                               [self
                                                   logFailure:
                                                       @"sign-in with GitHub (headful-lite) failed"
                                                        error:error];
                                               return;
                                             } else {
                                               [self logSuccess:@"sign-in with GitHub "
                                                                @"(headful-lite) succeeded."];
                                             }
                                             [self
                                                 showTypicalUIForUserUpdateResultsWithTitle:
                           ...
                           [[AppManager auth]
                               signInWithCredential:credential
                                         completion:^(FIRAuthDataResult *_Nullable authResult,
                                           ...
                                           [self hideSpinner:^{
                                             if (error) {
                                               [self logFailure:@"sign-in with Linkedin "
                                                                @"(headful-lite) failed"
                                                          error:error];
                                               return;
                                             } else {
                                               [self logSuccess:@"sign-in with Linkedin "
                                                                @"(headful-lite) succeeded."];
                                             }
                                             [self
                                                 showTypicalUIForUserUpdateResultsWithTitle:
                           ...
                           [[AppManager auth]
                               signInWithCredential:credential
                                         completion:^(FIRAuthDataResult *_Nullable authResult,
                                           ...
                                           [self hideSpinner:^{
                                             if (error) {
                                               [self logFailure:@"sign-in with Microsoft failed"
                                                          error:error];
                                               return;
                                             } else {
                                               [self logSuccess:@"sign-in with Microsoft "
                                                                @"(headful-lite) succeeded."];
                                             }
                                             [self
                                                 showTypicalUIForUserUpdateResultsWithTitle:
                               ...
                               signInWithCredential:credential
                                         completion:^(FIRAuthDataResult *_Nullable authResult,
                                           ...
                                           [self hideSpinner:^{
                                             if (error) {
                                               [self logFailure:
                                                         @"sign-in with Yahoo (headful-lite) failed"
                                                          error:error];
                                               return;
                                             } else {
                                               [self logSuccess:@"sign-in with Yahoo "
                                                                @"(headful-lite) succeeded."];
                                             }
                                             [self
                                                 showTypicalUIForUserUpdateResultsWithTitle:
...
- (ASAuthorizationAppleIDRequest *)appleIDRequestWithState:(NSString *)state
    API_AVAILABLE(ios(13.0)) {
  ASAuthorizationAppleIDRequest *request =
      [[[ASAuthorizationAppleIDProvider alloc] init] createRequest];
...
- (void)authorizationController:(ASAuthorizationController *)controller
  ...
  if ([appleIDCredential.state isEqualToString:@"signIn"]) {
    ...
    NSString *code = [[NSString alloc] initWithData:appleIDCredential.authorizationCode
                              ...
                              completion:^(NSError *_Nullable error) {
                                if (!error) {
                                  ...
                                  [user deleteWithCompletion:^(NSError *_Nullable error) {
                                    ...
                                    [self showTypicalUIForUserUpdateResultsWithTitle:@"Delete User"
                                                                               error:error];
                                  }];
                                } else {
                                  NSLog(@"%@", error.description);
