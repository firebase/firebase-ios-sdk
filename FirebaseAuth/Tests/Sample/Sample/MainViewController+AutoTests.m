...
static NSString *const kExpiredCustomTokenUrl =
    @"https://gcip-testapps.wl.r.appspot.com/expired_token";

static NSString *const kInvalidCustomToken = @"invalid custom token.";

static NSString *const kSafariGoogleSignOutMessagePrompt =
    @"This automated test assumes that no "
     "Google account is signed in on Safari, if your are not prompted for a password, sign out on "
     "Safari and rerun the test.";

static NSString *const kSafariFacebookSignOutMessagePrompt =
    @"This automated test assumes that no "
     "Facebook account is signed in on Safari, if your are not prompted for a password, sign out "
     "on "
     "Safari and rerun the test.";

static NSString *const kUnlinkAccountMessagePrompt =
    @"Sign into gmail with an email address "
                       ...
                       [auth.currentUser
                           getIDTokenForcingRefresh:NO
                                         completion:^(NSString *_Nullable token,
                                           ...
                                           [auth signOut:NULL];
                                           [auth
                                               signInWithCustomToken:expiredCustomToken
                                                          completion:^(
                                                              FIRAuthDataResult *_Nullable result,
                                                              NSError *_Nullable error) {
                                                            if (!error) {
                                                              [self
                                                                  logSuccess:@"sign-in with custom "
                                                                             @"token succeeded."];
                                                              [self logFailedTest:
                                                                        @"sign-in with an expired "
                                                                        @"custom token should NOT "
                                                                        @"succeed."];
                                                              return;
                                                            }
                                                            [self logFailure:@"sign-in with custom "
                                                                             @"token failed"
                                                                       error:error];
                                                            [auth
...
- (void)automatedSignInGoogle {
  [self
      showMessagePromptWithTitle:@"Sign In With Google"
                      ...
                      completion:^(BOOL userPressedOK, NSString *_Nullable userInput) {
                        ...
                        [self
                            signInWithProvider:[AuthProviders google]
                                      callback:^{
                                        [self
                                            logSuccess:@"sign-in with Google provider succeeded."];
                                        [auth signOut:NULL];
                                        [self
                                            signInWithProvider:[AuthProviders google]
                                                      callback:^{
                                                        [self logSuccess:@"sign-in with Google "
                                                                         @"provider succeeded."];
                                                        [self updateEmailPasswordWithCompletion:^{
                                                          [self
                                                              automatedSignInGoogleDisplayNamePhotoURL];
                                                        }];
                                                      }];
                                      }];
                      }];
...
- (void)automatedSignInGoogleDisplayNamePhotoURL {
  [self signInWithProvider:[AuthProviders google]
                  callback:^{
...
- (void)automatedSignInFacebook {
  [self
      showMessagePromptWithTitle:@"Sign In With Facebook"
                ...
                showCancelButton:NO
                      completion:^(BOOL userPressedOK, NSString *_Nullable userInput) {
                        ...
                        [self
                            signInWithProvider:[AuthProviders facebook]
                                      callback:^{
                                        [self logSuccess:
                                                  @"sign-in with Facebook provider succeeded."];
                                        [auth signOut:NULL];
                                        [self
                                            signInWithProvider:[AuthProviders facebook]
                                                      callback:^{
                                                        [self logSuccess:@"sign-in with Facebook "
                                                                         @"provider succeeded."];
                                                        [self updateEmailPasswordWithCompletion:^{
                                                          [self
                                                              automatedSignInFacebookDisplayNamePhotoURL];
                                                        }];
                                                      }];
                                      }];
                      }];
...
- (void)automatedPhoneNumberSignIn {
  ...
  [self
      commonPhoneNumberInputWithTitle:@"Phone for automation"
                           completion:^(NSString *_Nullable phone) {
                             [self
                                 signInWithPhoneNumber:phone
                                            completion:^(NSError *error) {
                                              if (error) {
                                                [self logFailedTest:@"Could not sign in with phone "
                                                                    @"number reCAPTCHA."];
                                              }
                                              [self logSuccess:@"sign-in with phone number "
                                                               @"reCAPTCHA test succeeded."];
                                              [auth signOut:NULL];
                                              [self
                                                  signInWithPhoneNumber:phone
                                                             completion:^(NSError *error) {
                                                                                    ...
                                                                                    FIRPhoneAuthProvider
                                                                                        ...
                                                                                        completion:^(
                                                                                          ...
                                                                                          if (error) {
                                                                                            [self
                                                                                                logFailedTest:
                                                                                                    @"Could not unlink phone number."];
                                                                                          }
                                                                                          [self
                                                                                              logSuccess:
                                                                                                  @"unlink phone number test succeeded."];
                                                                                          [self
                                                                                              linkPhoneNumber:
                                                                                                  phone
                                                                                                   completion:^(
                                                                                                     ...
                                                                                                     [self
                                                                                                         logSuccess:
                                                                                                             @"link phone number test succeeded."];
                                                                                                     [self
                                                                                                         log:@"FINISHED AUTOMATED MANUAL TEST FOR PHONE NUMBER SIGN IN."];
                                                                                                   }];
                                                                                        }];
                                                                          }];
                                                             }];
              ...
              [auth
                  signInWithCredential:credential
                            completion:^(FIRAuthDataResult *_Nullable result,
                              ...
                              [self log:@"FINISHED AUTOMATED MANUAL TEST FOR SIGN-IN WITH "
                                        @"EMAIL/PASSWORD."];
                              // Delete the user so that we can reuse the fake email address for
                              // subsequent tests.
                              [auth.currentUser deleteWithCompletion:^(NSError *_Nullable error) {
                                if (error) {
                                  [self logFailure:@"Failed to delete user" error:error];
                                  [self logFailedTest:@"Deleting a user that was recently "
                                                      @"signed-in should succeed."];
                                  return;
                                }
  ...
  [auth.currentUser
      updateEmail:kFakeEmail
       completion:^(NSError *_Nullable error) {
         ...
         [auth.currentUser
             updatePassword:kFakePassword
                 completion:^(NSError *_Nullable error) {
                   if (error) {
                     [self logFailure:@"update password failed" error:error];
                     [self
                         logFailedTest:@"Update password should succeed when properly signed-in."];
                     return;
                   }
                   ...
                   [auth signInWithCredential:credential
                                   completion:^(FIRAuthDataResult *_Nullable result,
                                                NSError *_Nullable error) {
                                     if (error) {
                                       [self logFailure:@"sign-in with Email/Password failed"
                                                  error:error];
                                       [self logFailedTest:
                                                 @"sign-in with Email/Password should succeed."];
                                       return;
                                     }
                                     [self logSuccess:@"sign-in with Email/Password succeeded."];
                                     // Delete the user so that we can reuse the fake email address
                                     // for subsequent tests.
                                     [auth.currentUser deleteWithCompletion:^(
                                                           NSError *_Nullable error) {
                                       if (error) {
                                         [self logFailure:@"Failed to delete user." error:error];
                                         [self logFailedTest:@"Deleting a user that was recently "
...
- (void)automatedSignInFacebookDisplayNamePhotoURL {
  [self signInWithProvider:[AuthProviders facebook]
                  callback:^{
                    [self updateDisplayNameAndPhotoURlWithCompletion:^{
...
- (void)automatedAccountLinking {
  ...
  [auth signOut:NULL];
  [self
      signInAnonymouslyWithCallback:
          ^(FIRAuthDataResult *_Nullable result, NSError *_Nullable error) {
            if (result.user) {
              NSString *anonymousUID = result.user.uid;
              [self
                  showMessagePromptWithTitle:@"Sign In Instructions"
                                     message:kUnlinkAccountMessagePrompt
                            showCancelButton:NO
                                  completion:^(BOOL userPressedOK, NSString *_Nullable userInput) {
                                    [[AuthProviders
                                        google] getAuthCredentialWithPresentingViewController:self
                                                                                     callback:
                                                                                         ^(FIRAuthCredential
                                                                                               *credential,
                                                                                           NSError *
                                                                                           ...
                                                                                           if (credential) {
                                                                                             [result
                                                                                                     .user linkWithCredential:
                                                                                                               credential
                                                                                                                   completion:
                                                                                                                       ^(FIRAuthDataResult *result, NSError
                                                                                                                                                        *
                                                                                                                                                            error) {
                                                                                                                         FIRUser *user =
                                                                                                                             result
                                                                                                                         ...
                                                                                                                         if (error) {
                                                                                                                           [self
                                                                                                                               logFailure:
                                                                                                                                   @"link auth provider failed"
                                                                                                                                    error:
                                                                                                                                        error];
                                                                                                                           [self
                                                                                                                               logFailedTest:
                                                                                                                                   @"Account needs to be linked to complete the test."];
                                                                                                                           return;
                                                                                                                         }
                                                                                                                         [self
                                                                                                                             logSuccess:
                                                                                                                                 @"link auth provider succeeded."];
                                                                                                                         if (user.isAnonymous) {
                                                                                                                           [self
                                                                                                                               logFailure:
                                                                                                                                   @"link auth provider failed, user still anonymous"
                                                                                                                                    error:
                                                                                                                                        error];
                                                                                                                           [self
                                                                                                                               logFailedTest:
                                                                                                                         ...
                                                                                                                         if (!
                                                                                                                           ...
                                                                                                                           [self
                                                                                                                               logFailedTest:
                                                                                                                                   @"link auth provider failed, UID's are different. Make sure "
                                                                                                                                    "you link an account that has NOT been Linked nor Signed-In before."];
                                                                                                                           return;
                                                                                                                         }
                                                                                                                         [self
                                                                                                                             log:@"FINISHED AUTOMATED MANUAL TEST FOR ACCOUNT LINKING."];
                                                                                                                       }];
                                                                                           }
                                                                                         }];
                                  }];
