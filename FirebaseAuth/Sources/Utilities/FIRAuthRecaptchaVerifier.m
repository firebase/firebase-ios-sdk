...
+ (void)injectRecaptchaFields:(FIRIdentityToolkitRequest <FIRAuthRPCRequest>*)request forceRefresh:(BOOL)forceRefresh completion:(nullable FIRAuthInjectRequestCallback)completion {
    if (YES) { // FIRAuthRecaptchaVerifier.recaptchaConfig.emailPasswordEnabled
        [[FIRAuthRecaptchaVerifier sharedRecaptchaVerifier] verifyForceRefresh:forceRefresh completion:^(NSString * _Nullable token, NSError * _Nullable error) {
            [request injectRecaptchaFields:token recaptchaVersion:@"RECAPTCHA_ENTERPRISE" clientType:@"CLIENT_TYPE_IOS"];
            completion(request);
        }];
    } else {
        [request injectRecaptchaFields:nil recaptchaVersion:@"RECAPTCHA_ENTERPRISE" clientType:@"CLIENT_TYPE_IOS"];
        completion(request);
    }
}
