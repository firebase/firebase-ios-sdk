...
@objc(FIRAuthNotificationManager) public class AuthNotificationManager: NSObject {
   ...
   */
  @objc public init(withApplication application: Application, appCredentialManager: AuthAppCredentialManager) {
    self.application = application
    self.appCredentialManager = appCredentialManager
    self.timeout = kProbingTimeout
  }
  ...
  @objc public func checkNotificationForwarding(withCallback callback: @escaping (Bool) -> Void) {
    ...
    DispatchQueue.main.async {
      ...
      if let delegate = self.application.delegate {
        delegate.application!(UIApplication.shared, didReceiveRemoteNotification: proberNotification) { _ in
        }
      } else {
        AuthLog.logWarning(code: "I-AUT000015", message: "The UIApplicationDelegate must handle " +
  ...
  @objc(canHandleNotification:) public func canHandle(notification: [AnyHashable: Any]) -> Bool {
    var stringDictionary: [String: Any]? = nil
