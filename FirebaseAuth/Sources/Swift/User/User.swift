...
@objc(FIRUser) open class User: NSObject, UserInfo {
  ...
  ///    account.
    ...
    ///  calling `reauthenticate(with:)`.
      ...
      /// - Parameter completion: Optionally; the block invoked when the user profile change has finished.
  @available(
    *,
    deprecated,
...
/// dictionary object will contain more detailed explanation that can be shown to the user.
  ...
  ///    the user has not signed in recently enough. To resolve, reauthenticate the user by
  ///     calling `reauthenticate(with:)`.
        /// - Parameter phoneNumberCredential: The new phone number credential corresponding to the phone number
...
/// - Returns: The `AuthDataResult` after the reauthentication.
  ...
  ///
  /// This method is available on iOS only.
        /// - Parameter provider: An instance of an auth provider used to initiate the reauthenticate flow.
        ...
        ///    will be used.
        /// - Parameter completion: Optionally; a block which is invoked when the reauthenticate flow finishes, or
  ...
  /// expired, or if `forceRefresh` is `true`.
      /// - Parameter forceRefresh: Forces a token refresh. Useful if the token becomes invalid for some reason
  ...
  /// `updatePassword(to:)` on `User`.
      ...
      /// - Parameter completion: Optionally; the block invoked when the unlinking is complete, or fails.
  ...
  /// - Parameter completion: Optionally; the block invoked when the request to send an email verification
  ...
  /// has not signed in recently enough. To resolve, reauthenticate the user by calling
  /// `reauthenticate(with:)`.
      /// - Parameter completion: Optionally; the block invoked when the request to delete the account is
  ...
  open func delete() async throws {
    return try await withCheckedThrowingContinuation { continuation in
      self.delete { error in
       ...
       /// - Parameter completion: Optionally; the block invoked when the request to send the verification
  ...
  private func updateEmail(email: String?,
    ...
    executeUserUpdateWithChanges(changeBlock: { user, request in
      ...
      /// - Parameter changeBlock: A block responsible for mutating a template @c FIRSetAccountInfoRequest
      /// - Parameter callback: A block to invoke when the change is complete. Invoked asynchronously on the
  ...
  private func setTokenService(tokenService: SecureTokenService,
    ...
    tokenService.fetchAccessToken(forcingRefresh: false) { token, error, tokenUpdated in
      ...
      /// - Parameter callback: Invoked when the request to getAccountInfo has completed, or when an error has
  ...
  /// Invoked asynchronously on the global work queue in the future.
    /// - Parameter phoneAuthCredential: The new phone number credential corresponding to the phone number
    ///   to be added to the Firebase account, if a phone number is already linked to the account this
    /// new phone number will replace it.
        /// - Parameter isLinkOperation: Boolean value indicating whether or not this is a link operation.
        /// - Parameter completion: Optionally; the block invoked when the user profile change has finished.
  ...
  private func signOutIfTokenIsInvalid(withError error: Error) {
    ...
    if code == AuthErrorCode.userNotFound.rawValue ||
      ...
      /// Retrieves the Firebase authentication token, possibly refreshing it if it has expired.
      /// - Parameter callback: The block to invoke when the token is available. Invoked asynchronously on the
