...
class InAppMessagingDisplayUITestsBase: XCTestCase {
  func waitForElementToAppear(_ element: XCUIElement, _ timeoutInSeconds: TimeInterval = 30) {
    let existsPredicate = NSPredicate(format: "exists == true")
    expectation(for: existsPredicate, evaluatedWith: element, handler: nil)
    waitForExpectations(timeout: timeoutInSeconds, handler: nil)
  }

  func waitForElementToDisappear(_ element: XCUIElement, _ timeoutInSeconds: TimeInterval = 30) {
