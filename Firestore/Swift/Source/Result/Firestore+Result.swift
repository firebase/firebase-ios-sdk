...
extension Firestore {
  ...
  ///
  /// Any value maybe be returned from the updateBlock. If the transaction is successfully committed, then the completion block will be passed that value. The updateBlock also has an `Error` out parameter. If this is set, then the transaction will not attempt to commit, and the given error will be passed to the completion block.
  ///
  ...
  ///   - result: The result of the transaction. On success it contains the value passed in the updateBlock, otherwise an `Error`.
  func runTransaction<T>(_ updateBlock: @escaping (_ transaction: Transaction) throws -> T, completion: @escaping (Result<T, Error>) -> Void) {
...
private func mapResultClosure<T>(_ completion: @escaping (_ result: Result<T, Error>) -> Void) -> ((Any?, Error?) -> Void) {
  { value, error in
    if let value = value {
