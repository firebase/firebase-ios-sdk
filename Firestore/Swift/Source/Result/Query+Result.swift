...
extension Query {
  ...
  func getDocuments(source: FirestoreSource = .default, completion: @escaping (_ result: Result<QuerySnapshot, Error>) -> Void) {
  ...
  func addSnapshotListener(includeMetadataChanges: Bool = false, listenerHandler: @escaping (_ result: Result<QuerySnapshot, Error>) -> Void) -> ListenerRegistration {
    addSnapshotListener(includeMetadataChanges: includeMetadataChanges, listener: mapResultClosure(listenerHandler))
  }
}
