...
class SnapshotListenerSourceTests: FSTIntegrationTestCase {
  ...
  func testSharesServerSideUpdatesWhenListeningToBothCacheAndDefault() throws {
    ...
    func testListenToDocumentsWithVectors() throws {
        ...
        let registration = collection.whereField("purpose", isEqualTo: "vector tests").addSnapshotListener(eventAccumulator.valueEventHandler)
        ...
        XCTAssertEqual(querySnap.documents[0].data()["vector0"] as! VectorValue, FieldValue.vector([0.0]))
        XCTAssertEqual(querySnap.documents[0].data()["vector1"] as! VectorValue, FieldValue.vector([1, 2, 3.99]))
        ...
        XCTAssertEqual(querySnap.documents[0].data()["vector0"] as! VectorValue, FieldValue.vector([0.0]))
        XCTAssertEqual(querySnap.documents[0].data()["vector1"] as! VectorValue, FieldValue.vector([1, 2, 3.99]))
        XCTAssertEqual(querySnap.documents[0].data()["vector2"] as! VectorValue, FieldValue.vector([0.0, 0, 0]))
        ...
        XCTAssertEqual(querySnap.documents[0].data()["vector1"] as! VectorValue, FieldValue.vector([1, 2, 3.99]))
        XCTAssertEqual(querySnap.documents[0].data()["vector2"] as! VectorValue, FieldValue.vector([0.0, 0, 0]))
        XCTAssertEqual(querySnap.documents[0].data()["vector3"] as! VectorValue, FieldValue.vector([-1, -200, -999.0]))
