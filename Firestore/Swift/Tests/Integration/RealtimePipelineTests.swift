...
class RealtimePipelineIntegrationTests: FSTIntegrationTestCase {
  ...
  func testCanListenToCache() async throws {
    ...
    actor IteratorWrapper {
      var iterator: AsyncThrowingStream<RealtimePipeline.Snapshot, Error>.Iterator
      init(_ iterator: AsyncThrowingStream<RealtimePipeline.Snapshot, Error>.Iterator) {
        self.iterator = iterator
      }

      func next() async throws -> RealtimePipeline.Snapshot? {
        var localIterator = iterator
        defer { iterator = localIterator }
        return try await localIterator.next()
      }
    }
    let wrapper = IteratorWrapper(stream.makeAsyncIterator())

    let firstSnapshot = try await wrapper.next()
    XCTAssertEqual(firstSnapshot!.metadata.isFromCache, true)
    XCTAssertEqual(firstSnapshot!.results().count, 3)
    XCTAssertEqual(firstSnapshot!.results().first?.get("title") as? String, "Dune")
    XCTAssertEqual(firstSnapshot!.results()[1].get("title") as? String, "Pride and Prejudice")
