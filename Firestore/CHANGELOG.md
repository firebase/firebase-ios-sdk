...
- [feature] Added a new `Timestamp` class to represent timestamp fields,
  ...
  Dates, set the newly-added flag `areTimestampsInSnapshotsEnabled` to `true` in
  `FirestoreSettings`. Note that the current behavior (`DocumentSnapshot`s
  returning system Dates) will be removed in a future release. Using
  `Timestamp`s avoids rounding errors (system Date is stored as a floating-point
  value, so the value read back from a `DocumentSnapshot` might be slightly
  different from the value written).
