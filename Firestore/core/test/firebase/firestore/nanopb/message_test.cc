...
grpc::ByteBuffer GoodProto() {
  Proto proto{};

  Serializer serializer{DatabaseId{"p", "d"}};
  // A couple of fields should be enough -- these tests are primarily concerned
  // with ownership, not parsing.
  proto.stream_id = serializer.EncodeString("stream_id");
  proto.stream_token = serializer.EncodeString("stream_token");
