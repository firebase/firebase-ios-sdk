...
TEST_F(LevelDbLocalStoreTest, IndexAutoCreationDoesnotWorkWithMultipleInequality) {
  core::Query query = testutil::Query("coll").AddingFilter(Filter("field1", "<", 5)).AddingFilter(Filter("field2", "<", 5));
  int target_id = AllocateQuery(query);
  ...
  ApplyRemoteEvent(
      AddedRemoteEvent(Doc("coll/b", 10, Map("field1", 8, "field2", 2)), {target_id}));
  ApplyRemoteEvent(
      AddedRemoteEvent(Doc("coll/c", 10, Map("field1", "string", "field2", 2)), {target_id}));
  ApplyRemoteEvent(AddedRemoteEvent(
      Doc("coll/d", 10, Map("field1", 2)), {target_id}));
  ApplyRemoteEvent(AddedRemoteEvent(
