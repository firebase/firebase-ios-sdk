...
std::string ByteBufferToString(const grpc_adapt::ByteBuffer& buffer);
grpc_adapt::ByteBuffer MakeByteBuffer(const std::string& str);
 ...
 *    successfully.
 * CompletionEndState{Type::Finish, grpc_adapt::Status{grpc_adapt::DATA_LOSS,
...
struct CompletionEndState {
  ...
  CompletionEndState(GrpcCompletion::Type type,
                     const grpc_adapt::Status& status)
 ...
 private:
  ...
  absl::optional<grpc_adapt::ByteBuffer> maybe_message_;
  absl::optional<grpc_adapt::Status> maybe_status_;
};
...
class FakeGrpcQueue {
 public:
  ...
  explicit FakeGrpcQueue(grpc_adapt::CompletionQueue* grpc_queue);
  ...
  grpc_adapt::CompletionQueue* queue() {
    return grpc_queue_;
  }
...
class GrpcStreamTester {
 public:
   ...
   */
  void ForceFinish(grpc_adapt::ClientContext* context,
