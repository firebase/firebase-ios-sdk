...
class GrpcUnaryCallTest : public testing::Test {
 public:
  ...
  void StartCall() {
    call->Start([this](const StatusOr<grpc_adapt::ByteBuffer>& result) {
  ...
  grpc_adapt::ByteBuffer response;
...
TEST_F(GrpcUnaryCallTest, CallbackCanDestroyCallOnSuccess) {
  worker_queue->EnqueueBlocking([&] {
    call->Start(
        [this](const StatusOr<grpc_adapt::ByteBuffer>&) { call.reset(); });
  });
...
TEST_F(GrpcUnaryCallTest, CallbackCanDestroyCallOnError) {
  ...
  grpc_adapt::Status error_status{grpc_adapt::StatusCode::UNAVAILABLE, ""};
  EXPECT_NE(call, nullptr);
