...
std::string GetFirestoreErrorName(enum Error error) {
  return GetGrpcErrorCodeName(static_cast<grpc_adapt::StatusCode>(error));
}
...
std::string ByteBufferToString(const grpc_adapt::ByteBuffer& buffer) {
  ...
  // TODO(GRPC_SWIFT): Add Range Support
  /*
...
grpc_adapt::ByteBuffer MakeByteBuffer(const std::string& str) {
  grpc_adapt::Slice slice{str};
  return grpc_adapt::ByteBuffer(&slice, 1);
}
...
FakeGrpcQueue::FakeGrpcQueue(grpc_adapt::CompletionQueue* grpc_queue)
...
void GrpcStreamTester::ForceFinish(grpc_adapt::ClientContext* context,
                                   const CompletionCallback& callback) {
  // gRPC allows calling `TryCancel` more than once.
  context->TryCancel();
