 ...
 public:
  ...
  void StartReader() {
    worker_queue->EnqueueBlocking([&] {
      reader->Start(
          [this](const StatusOr<std::vector<grpc_adapt::ByteBuffer>>& result) {
...
TEST_F(GrpcStreamingReaderTest, OneSuccessfulRead) {
  ...
  // ForceFinish({{Type::Finish, grpc_adapt::Status::OK}});
...
TEST_F(GrpcStreamingReaderTest, ErrorOnSecondRead) {
  ...
  ForceFinish({{Type::Finish,
                grpc_adapt::Status{grpc_adapt::StatusCode::DATA_LOSS, ""}}});
  ASSERT_TRUE(status.has_value());
...
TEST_F(GrpcStreamingReaderTest, CallbackCanDestroyReaderOnSuccess) {
  worker_queue->EnqueueBlocking([&] {
    reader->Start([this](const StatusOr<std::vector<grpc_adapt::ByteBuffer>>&) {
      reader.reset();
    });
  });
  ...
  // EXPECT_NO_THROW(ForceFinish({{Type::Finish, grpc_adapt::Status::OK}}));
...
TEST_F(GrpcStreamingReaderTest, CallbackCanDestroyReaderOnError) {
  ...
  grpc_adapt::Status error_status{grpc_adapt::StatusCode::DATA_LOSS, ""};
  EXPECT_NE(reader, nullptr);
