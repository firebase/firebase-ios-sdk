...
size_t Query::Hash() const {
  return util::Hash(firestore_.get(), [query() hash]);
}
...
void Query::ValidateOrderByField(const FieldPath& orderByField,
  ...
  if (orderByField != inequalityField) {
    ThrowInvalidArgument(
        "Invalid query. You have a where filter with an inequality (lessThan, "
        "lessThanOrEqual, greaterThan, or greaterThanOrEqual) on field '%s' "
        "and so you must also use '%s' as your first queryOrderedBy field, but "
        "your first queryOrderedBy is currently on field '%s' instead.",
