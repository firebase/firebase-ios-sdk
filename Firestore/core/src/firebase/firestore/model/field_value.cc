...
FieldValue& FieldValue::operator=(const FieldValue& value) {
  ...
  switch (tag_) {
    ...
    case Type::Object: {
      // copy-and-swap
      std::map<std::string, const FieldValue> tmp = value.object_value_;
...
FieldValue FieldValue::ObjectValue(
    const std::map<std::string, const FieldValue>& value) {
  std::map<std::string, const FieldValue> copy(value);
  return ObjectValue(std::move(copy));
}

FieldValue FieldValue::ObjectValue(
    std::map<std::string, const FieldValue>&& value) {
  FieldValue result;
...
void FieldValue::SwitchTo(const Type type) {
  ...
  switch (tag_) {
    ...
    case Type::Array:
      ...
      break;
    case Type::Object:
      new (&object_value_) std::map<std::string, const FieldValue>();
      break;
    default: {}  // The other types where there is nothing to worry about.
  }
}
