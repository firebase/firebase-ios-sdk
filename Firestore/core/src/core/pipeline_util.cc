...
#include "expressions_eval.h"
#include "pipeline_run.h"  // Contains RunPipeline, EvaluateContext
...
bool QueryOrPipeline::MatchesAllDocuments() const {
  if (IsPipeline()) {
    for (const auto& stage : pipeline().rewritten_stages()) {
      ...
      // TODO: Add checks for other filtering stages like Aggregate,
...
bool QueryOrPipeline::has_limit() const {
  if (this->IsPipeline()) {
    for (const auto& stage : this->pipeline().rewritten_stages()) {
      ...
      // TODO: need to check for other stages that could have a limit, like
      // findNearest
    }
    ...
    for (size_t index = 0; index < sub_end; ++index) {
      if (index < sub_end - 1) {
        conditions.push_back(std::make_shared<api::FunctionExpr>(
            "eq", std::vector<std::shared_ptr<api::Expr>>{
