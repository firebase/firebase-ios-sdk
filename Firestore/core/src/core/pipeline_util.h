...
class TargetOrPipeline {
 public:
  ...
  TargetOrPipeline(const Target& target) : data_(target) {
  }  // NOLINT
  TargetOrPipeline(Target&& target) : data_(std::move(target)) {
  }  // NOLINT
  TargetOrPipeline(const api::RealtimePipeline& pipeline)  // NOLINT
      : data_(pipeline) {
  }
  TargetOrPipeline(api::RealtimePipeline&& pipeline)  // NOLINT
...
class QueryOrPipeline {
 public:
  ...
  QueryOrPipeline(const Query& query) : data_(query) {
  }  // NOLINT
  QueryOrPipeline(Query&& query) : data_(std::move(query)) {
  }  // NOLINT
  QueryOrPipeline(const api::RealtimePipeline& pipeline)  // NOLINT
      : data_(pipeline) {
  }
  QueryOrPipeline(api::RealtimePipeline&& pipeline)  // NOLINT
      : data_(std::move(pipeline)) {
  }
