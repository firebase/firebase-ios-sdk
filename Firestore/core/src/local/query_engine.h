 ...
 *
 * There are some cases where this specific optimization is not guaranteed to
 * produce the same results as full collection scans. In these cases, query
...
class QueryEngine {
 public:
   ...
   */
  virtual void SetDependencies(LocalDocumentsView* local_documents);
 ...
 private:
  ...
  /**
   * Performs an indexed query that evaluates the query based on a known
   * IndexType. Returns null if an index is not available.
   */
  const absl::optional<model::DocumentMap> PerformQueryUsingIndexWithKnownType(
      const core::Query& query) const;
