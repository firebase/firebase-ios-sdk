...
enum AILog {
  ...
  static let label = "com.google.firebase.GeneratedFirebaseAI"

  static let logger = Logger(label: label)

  /// The argument required to enable additional logging.
  static let enableArgumentKey = "-FIRDebugEnabled"

  static func log(level: Logger.Level, code: MessageCode, _ message: String) {
    let messageCode = String(format: "I-VTX%06d", code.rawValue)
    logger.log(level: level, "\(messageCode) \(message)")
  }
