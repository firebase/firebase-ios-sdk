...
class FunctionsStreamTests: XCTestCase {
  ...
  func testStream_cancellation_resourceLeak() async throws {
    ...
    let functions = Functions(projectID: "test-project", region: "us-central1", customDomain: nil, auth: nil, messaging: nil, appCheck: nil)
  ...
  func testStream_failure_midStreamError_throwsError() async throws {
    ...
    let throwsExpectation =
      XCTestExpectation(description: "Stream should throw dataLoss wrapping URLError(.networkConnectionLost)")
    ...
    Task {
      do {
        ...
        XCTFail("Stream should not finish successfully; it should throw a mid-stream network error.")
