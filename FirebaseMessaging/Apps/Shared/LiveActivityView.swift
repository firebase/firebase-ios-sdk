...
struct LiveActivityView: View {
            ...
            List{
                ForEach(activityTokenDict.keys.sorted(),id: \.self){ key in
                    VStack{
                        ...
                        Text("Push Token: " + activityTokenDict[key]!).frame(maxWidth: .infinity, alignment: .leading)
                    }.padding(10)
                }
            }
        }.onAppear{
    ...
    private func refreshAcitivtyList(){
        Task{
            ...
            if(ptsToken != nil){
                ...
                Task{
                    for await ptsToken in Activity<SampleLiveActivityAttributes>.pushToStartTokenUpdates {
                        let ptsTokenString = getFormatedToken(token: ptsToken)
                        activityTokenDict["PTS"] = ptsTokenString
                        refreshAcitivtyList()
                    }
    ...
    private func startLiveActivity(supportPush:Bool) {
        let lAttributes = SampleLiveActivityAttributes(lvInstanceNumber: Date().timeIntervalSince1970)
    ...
    func cancelAllRunningActivities() {
        Task{
            for activity in Activity<SampleLiveActivityAttributes>.activities {
                let initialContentState = SampleLiveActivityAttributes.ContentState(status: .started)
