...
readonly PROTO_DIR="${REPO_DIR}/FirebaseMessaging/ProtoSupport/Protos"
readonly PROTOGEN_DIR="${REPO_DIR}/FirebaseMessaging/Sources"
...
echo "------"
echo "${PROTO_DIR}"
echo "${NANOPB_TEMPDIR}/${NANOPB_BIN_DIR}/generator"
echo "${PROTOGEN_DIR}" 
echo "------"

echo "Generating nanopb protos..."
python "${REPO_DIR}/FirebaseMessaging/ProtoSupport/nanopb_build_protos.py" \
  --nanopb \
