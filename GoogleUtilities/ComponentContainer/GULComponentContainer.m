...
#import "Private/GULLibrary.h"

#import <GoogleUtilities/GULLogger.h>

static GULLoggerService kGULComponentContainer = @"[GoogleUtilities/ComponentContainer]";

NS_ASSUME_NONNULL_BEGIN

@interface GULComponentContainer ()
...
@implementation GULComponentContainer
...
- (instancetype)initWithContext:(nullable id)context
  ...
  if (self) {
    ...
    _components = [NSMutableDictionary<NSString *, GULComponentCreationBlock> dictionary];
...
- (void)populateComponentsFromRegisteredClasses:(NSSet<Class> *)classes withContext:(id)context {
  ...
  for (Class<GULLibrary> klass in classes) {
    // Loop through all the components being registered and store them as appropriate.
    // Classes which do not provide functionality should use a dummy GULComponentRegistrant
    // protocol.
    for (GULComponent *component in [klass componentsToRegister]) {
      ...
      // array yet.
      if (component.instantiationTiming == GULInstantiationTimingAlwaysEager) {
...
- (nullable id)instantiateInstanceForProtocol:(Protocol *)protocol
                                    withBlock:(GULComponentCreationBlock)creationBlock {
  if (!creationBlock) {
    return nil;
  }
...
- (nullable id)instanceForProtocol:(Protocol *)protocol {
  ...
  @synchronized(self) {
    ...
    if (!cachedInstance) {
      ...
      GULComponentCreationBlock creationBlock = self.components[protocolName];
