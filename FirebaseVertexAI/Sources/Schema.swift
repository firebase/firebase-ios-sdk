...
public class Schema {
  ...
  ///
  /// This schema instructs the model to produce data of type `number` with the `format` `"float"`,
  /// which is suitable for decoding into a Swift `Float` (or `Float?`, if `nullable` is set to
  /// `true`).
  ///
  ...
  ///
  /// This schema instructs the model to produce data of type `number` with the `format` `"double"`,
  /// which is suitable for decoding into a Swift `Double` (or `Double?`, if `nullable` is set to
  /// `true`).
  ///
  /// > Important: This `Schema` provides a hint to the model that it should generate a
  ...
  /// `string` and values of any other data type (including nested `object`s). This data is suitable
  /// for decoding into Swift keyed collection types, including `Dictionary`, or other custom
