...
- (FTrackedQueryManager *)newManagerWithStorageEngine:(id<FStorageEngine>)storageEngine {
  return
      [[FTrackedQueryManager alloc] initWithStorageEngine:storageEngine clock:[FSystemClock clock]];
}
...
- (void)testSetQueriesComplete {
  ...
  [manager
      setQueryActive:[[FQuerySpec alloc] initWithPath:PATH(@"elsewhere") params:SAMPLE_PARAMS]];
  ...
  XCTAssertTrue(
      [manager findTrackedQuery:[[FQuerySpec alloc] initWithPath:PATH(@"foo") params:SAMPLE_PARAMS]]
          .isComplete);
...
- (void)testPruneOldQueries {
  ...
  forest = [manager
      pruneOldQueries:[[FTestCachePolicy alloc] initWithPercent:1 maxQueries:NSUIntegerMax]];
  [self checkPruneForest:forest
...
- (void)testPruneQueriesOverMaxSize {
  ...
  for (NSUInteger i = 0; i < 10; i++) {
    [manager setQueryActive:[FQuerySpec
                                defaultQueryAtPath:PATH(([NSString stringWithFormat:@"%lu", i]))]];
    [manager
  ...
  FPruneForest *forest =
      [manager pruneOldQueries:[[FTestCachePolicy alloc] initWithPercent:0.2 maxQueries:6]];
  [self checkPruneForest:forest
...
- (void)testPruneQueriesOverMaxSizeUsingPercent {
  ...
  FPruneForest *forest =
      [manager pruneOldQueries:[[FTestCachePolicy alloc] initWithPercent:0.6 maxQueries:6]];
  [self checkPruneForest:forest
             pathsToKeep:@[ @"6", @"7", @"8", @"9" ]
