...
- (void)testChildrenNodeEqualsHashCode {
  id<FNode> nodeOne = [FSnapshotUtilities nodeFrom:@{
    @"one" : @1,
    @"two" : @2,
    @".priority" : @"prio"
  }];
  ...
  XCTAssertFalse(
      [[nodeOne updateImmediateChild:@"one" withNewChild:[FEmptyNode emptyNode]] isEqual:nodeOne]);
...
- (void)testUpdatingEmptyChildrenDoesntOverwriteLeafNode {
  ...
  XCTAssertEqualObjects(
      node, [node updateChild:[FPath pathWithString:@"child"] withNewChild:[FEmptyNode emptyNode]]);
...
- (void)testUpdatingPrioritiesOnEmptyNodesIsANoOp {
  ...
  XCTAssertTrue([[[[FEmptyNode emptyNode] updateImmediateChild:@".priority" withNewChild:priority]
      getPriority] isEmpty]);

  id<FNode> valueNode = [FSnapshotUtilities nodeFrom:@"value"];
  FPath *childPath = [FPath pathWithString:@"child"];
  id<FNode> reemptiedChildren =
      [[[FEmptyNode emptyNode] updateChild:childPath withNewChild:valueNode]
  ...
  XCTAssertTrue([[[reemptiedChildren updateChild:[FPath pathWithString:@".priority"]
                                    withNewChild:priority] getPriority] isEmpty]);
  XCTAssertTrue([[[reemptiedChildren updateImmediateChild:@".priority" withNewChild:priority]
      getPriority] isEmpty]);
}
...
- (void)testDeletingLastChildFromChildrenNodeRemovesPriority {
  ...
  id<FNode> valueNode = [FSnapshotUtilities nodeFrom:@"value"];
  FPath *childPath = [FPath pathWithString:@"child"];
  id<FNode> withPriority = [[[FEmptyNode emptyNode] updateChild:childPath withNewChild:valueNode]
