...
- (FIRDataSnapshot*)snapshotFor:(id)jsonDict {
  ...
  FRepoInfo* repoInfo =
      [[FRepoInfo alloc] initWithHost:@"example.com" isSecure:NO withNamespace:@"default"];
...
- (void)testUpdateImmediateChildReturnsANewChildrenNode {
  FLeafNode* x =
      [[FLeafNode alloc] initWithValue:@"test"
                          withPriority:[FSnapshotUtilities nodeFrom:[NSNumber numberWithInt:42]]];
  FChildrenNode* y =
      [x updateImmediateChild:@"test" withNewChild:[[FLeafNode alloc] initWithValue:@"foo"]];
...
- (void)testUpdatingEmptyChildDoesntOverwriteLeafNode {
  ...
  XCTAssertEqualObjects(
      node,
      [node updateChild:[[FPath alloc] initWith:@".priority"] withNewChild:[FEmptyNode emptyNode]],
      @"Update should not affect node.");
  XCTAssertEqualObjects(
      node,
      [node updateChild:[[FPath alloc] initWith:@"child"] withNewChild:[FEmptyNode emptyNode]],
      @"Update should not affect node.");
...
- (void)testUnicodeEquality {
  FNamedNode* node1 =
      [[FNamedNode alloc] initWithName:@"a" andNode:[[FLeafNode alloc] initWithValue:@"유주연"]];
  FNamedNode* node2 =
  ...
  XCTAssertEqual([index compareNamedNode:node1 toNamedNode:node2],
                 -[index compareNamedNode:node2 toNamedNode:node1]);
}
