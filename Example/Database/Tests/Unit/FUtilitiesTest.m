...
- (void)testDefaultCacheSizeIs10MB {
  XCTAssertEqual([FIRDatabaseReference defaultConfig].persistenceCacheSizeBytes,
                 (NSUInteger)10 * 1024 * 1024);
  XCTAssertEqual([FIRDatabaseConfig configForName:@"test-config"].persistenceCacheSizeBytes,
                 (NSUInteger)10 * 1024 * 1024);
}
