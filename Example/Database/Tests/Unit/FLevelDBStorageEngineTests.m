...
- (void)testExtremeDoublesAsServerCache {
  ...
  FLevelDBStorageEngine *engine = [self cleanStorageEngine];
  [engine updateServerCache:NODE((
                                @{ @"works" : @"value",
                                   @"fails" : @(2.225073858507201e-308) }))
                     atPath:PATH(@"foo")
                      merge:NO];
...
- (void)testExtremeDoublesAsTrackedQuery {
  ...
  FTrackedQuery *doesNotWork =
      [[FTrackedQuery alloc] initWithId:0
                                  query:[[FQuerySpec alloc] initWithPath:PATH(@"foo") params:params]
                                lastUse:0
                               isActive:NO];
  FTrackedQuery *doesWork =
      [[FTrackedQuery alloc] initWithId:1
...
- (void)testExtremeDoublesAsUserWrites {
  ...
  [engine saveUserMerge:[[FCompoundWrite emptyWrite] addWrite:tinyDouble atPath:PATH(@"bar")]
                 atPath:PATH(@"foo")
                writeId:2];
  ...
  XCTAssertEqualObjects([engine userWrites],
                        @[ [[FWriteRecord alloc] initWithPath:PATH(@"other")
