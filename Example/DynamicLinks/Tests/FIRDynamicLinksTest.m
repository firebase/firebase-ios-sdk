 ...
 */
NSString *kEncodedComplicatedURLString = @"https%3A%2F%2Fwww.google.com%252Fmaps%2Fplace%2FTokyo+Am"
...
NSString *kDecodedComplicatedURLString = @"https://www.google.com%2Fmaps/place/Tokyo+American+Club/"
...
static void SwizzleDynamicLinkNetworkingWithMock() {
  ...
  [[linkResolver stub] resolveShortLink:OCMOCK_ANY
...
- (void)testFactoryMethodReturnsProperClassObject {
  ...
  XCTAssertEqualObjects([service class],
                        [FIRDynamicLinks class],
                        @"Factory returned incorrect class object");
}

- (void)testURLScheme_LaunchOptionsWithCustomSchemeURL {
  NSString *deepLinkString = [NSString stringWithFormat:kStructuredLinkFmtDeepLink,
...
- (void)testCustomScheme_FirstTimeOpenedFromFingerprintCodepathShouldGetWeakMatch {
  ...
  [self.service setUpWithLaunchOptions:nil
                          ...
                          userDefaults:self.userDefaults];
  FIRDynamicLink *dynamicLink = [self.service dynamicLinkFromCustomSchemeURL:url];
  XCTAssertEqual(dynamicLink.matchConfidence, FIRDynamicLinkMatchConfidenceWeak,
...
- (void)testHandleUniversalLinkCompletionReturnsNoForNonDDL {
  ...
  [urlStrings enumerateObjectsUsingBlock:^(NSString * _Nonnull urlString,
                                           NSUInteger idx,
                                           BOOL * _Nonnull stop) {
    NSURL *url = [NSURL URLWithString:urlString];
    BOOL handled = [self.service handleUniversalLink:url
