...
struct SpecRepoBuilder: ParsableCommand {
  ...
  func createSpecPushFlags(of sdk: String, from manifest: [Pod]) -> [String] {
    var flags = sdk == "Firebase" ? Constants.umbrellaPodFlags : Constants.flags
  ...
  mutating func run() throws {
    ...
    let podReleaseConfig = FirebaseManifest.shared.pods
    ...
    for pod in specFileDict.depInstallOrder {
      ...
      podExitCode = pushPodspec(
        ...
        sdkRepo: sdkRepo,
        sources: podSources,
        flags: createSpecPushFlags(of: pod, from: podReleaseConfig)
      )
      if podExitCode != 0 {
        exitCode = 1
