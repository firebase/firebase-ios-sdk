...
package struct InteractionCreatedEvent: Codable, Sendable, Equatable, Hashable {
  ...
  /// Creates a new `InteractionCreatedEvent`.
  ///
  /// - Parameters:
  ///   - eventId: The event_id token to be used to resume the interaction stream, from
  ///   - interaction: Partial interaction resource emitted when the stream is created.
  package init(
    eventId: String? = nil,
    interaction: InteractionSseEventInteraction?
  ) {
    self.eventId = eventId
    self.eventType = "interaction.created"
