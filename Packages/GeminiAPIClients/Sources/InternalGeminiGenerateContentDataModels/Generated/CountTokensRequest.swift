...
public struct CountTokensRequest: Codable, Sendable, Equatable, Hashable {
  ...
  public let instances: [JSONValue]?
  ...
  /// (behavior varies by backend). For more details, see ``contents``.
  ///   - generateContentRequest: Optional. The overall input given to the `Model`. This includes
  /// the prompt as well as (Gemini Developer API only). For more details, see
  /// ``generateContentRequest``.
  ///   - model: Optional. The name of the publisher model requested to serve the prediction.
  /// (Gemini Enterprise Agent Platform only). For more details, see ``model``.
  ///   - instances: Optional. The instances that are the input to token counting call. (Gemini
  /// Enterprise Agent Platform only). For more details, see ``instances``.
  ///   - systemInstruction: Optional. The user provided system instructions for the model. (Gemini
  /// Enterprise Agent Platform only). For more details, see ``systemInstruction``.
  ///   - tools: Optional. A list of `Tools` the model may use to generate the next response.
  /// (Gemini Enterprise Agent Platform only). For more details, see ``tools``.
  ///   - generationConfig: Optional. Generation config that the model will use to generate the
  /// response. (Gemini Enterprise Agent Platform only). For more details, see ``generationConfig``.
