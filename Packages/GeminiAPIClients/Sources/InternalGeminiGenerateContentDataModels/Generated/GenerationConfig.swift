...
///
/// Type: `GoogleAiGenerativelanguageV1betaGenerationConfig`
///
...
///
/// This message contains all the parameters that control how the model generates
/// content. It allows you to influence the randomness, length, and structure of
/// the output.
public struct GenerationConfig: Codable, Sendable, Equatable, Hashable {
  ...
  /// returned.
  ///
  /// Log probabilities are the logarithm of the probability of a token appearing
  ...
  ///
  /// - Parameters:
  ///   - candidateCount: Optional. Number of generated responses to return. If unset, this will
  /// default (behavior varies by backend). For more details, see ``candidateCount``.
  ///   - stopSequences: Optional. The set of character sequences (up to 5) that will stop output
  /// generation. (behavior varies by backend). For more details, see ``stopSequences``.
  ///   - maxOutputTokens: Optional. The maximum number of tokens to include in a response
  /// candidate. (behavior varies by backend). For more details, see ``maxOutputTokens``.
  ///   - temperature: Optional. Controls the randomness of the output. (behavior varies by
  /// backend). For more details, see ``temperature``.
  ///   - topP: Optional. The maximum cumulative probability of tokens to consider when sampling.
  /// (behavior varies by backend). For more details, see ``topP``.
  ///   - topK: Optional. The maximum number of tokens to consider when sampling. (behavior varies
  /// by backend). For more details, see ``topK``.
  ///   - seed: Optional. Seed used in decoding. If not set, the request uses a randomly generated
  /// (behavior varies by backend). For more details, see ``seed``.
  ///   - responseMimeType: Optional. MIME type of the generated candidate text. (behavior varies by
  /// backend). For more details, see ``responseMimeType``.
  ///   - responseSchema: Optional. Output schema of the generated candidate text. Schemas must be a
  /// (behavior varies by backend). For more details, see ``responseSchema``.
  ///   - responsejsonschema: Optional. Output schema of the generated response. This is an
  /// alternative to (Gemini Developer API only). For more details, see ``responsejsonschema``.
  ///   - responseJsonSchema: Optional. An internal detail. Use `responseJsonSchema` rather than
  /// this field. (behavior varies by backend). For more details, see ``responseJsonSchema``.
  ///   - presencePenalty: Optional. Presence penalty applied to the next token's logprobs if the
  /// token has (behavior varies by backend). For more details, see ``presencePenalty``.
  ///   - frequencyPenalty: Optional. Frequency penalty applied to the next token's logprobs,
  /// multiplied by the (behavior varies by backend). For more details, see ``frequencyPenalty``.
  ///   - responseLogprobs: Optional. If true, export the logprobs results in response. (behavior
  /// varies by backend). For more details, see ``responseLogprobs``.
  ///   - logprobs: Optional. Only valid if response_logprobs=True. (behavior varies by backend).
  /// For more details, see ``logprobs``.
  ///   - enableEnhancedCivicAnswers: Optional. Enables enhanced civic answers. It may not be
  /// available for all models. (Gemini Developer API only). For more details, see
  /// ``enableEnhancedCivicAnswers``.
  ///   - responseModalities: Optional. The requested modalities of the response. Represents the set
  /// of modalities (behavior varies by backend). For more details, see ``responseModalities``.
  ///   - speechConfig: Optional. The speech generation config.
  ///   - thinkingConfig: Optional. Config for thinking features. (behavior varies by backend). For
  /// more details, see ``thinkingConfig``.
  ///   - imageConfig: Optional. Config for image generation. (behavior varies by backend). For more
  /// details, see ``imageConfig``.
  ///   - mediaResolution: Optional. If specified, the media resolution specified will be used.
  /// (behavior varies by backend). For more details, see ``mediaResolution``.
  ///   - enableAffectiveDialog: Optional. If enabled, the model will detect emotions and adapt its
  /// responses (behavior varies by backend). For more details, see ``enableAffectiveDialog``.
  ///   - responseFormat: Optional. Configuration for the response output format. Allows specifying
  /// output (behavior varies by backend). For more details, see ``responseFormat``.
  ///   - audioTimestamp: Optional. If enabled, audio timestamps will be included in the request to
  /// (Gemini Enterprise Agent Platform only). For more details, see ``audioTimestamp``.
