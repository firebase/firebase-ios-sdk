...
extension GenAITypes.JSONValue: Decodable {
  public init(from decoder: Decoder) throws {
    ...
    if container.decodeNil() {
      ...
      self = .bool(boolValue)
    } else if let objectValue = try? container.decode(GenAITypes.JSONObject.self) {
      self = .object(objectValue)
    } else if let arrayValue = try? container.decode([GenAITypes.JSONValue].self) {
...
extension GenAITypes.JSONValue: Encodable {
