...
public extension GenAITypes {
  ...
  struct Candidate: Codable, Sendable, Equatable, Hashable {
    ...
    ///   - finishMessage: Optional. Output only. Details the reason why the model stopped
    /// generating tokens. (behavior varies by backend). For more details, see ``finishMessage``.
    ///   - safetyRatings: List of ratings for the safety of a response candidate. (behavior varies
    /// by backend). For more details, see ``safetyRatings``.
    ...
    ///   - tokenCount: Output only. Token count for this candidate. (Gemini Developer API only).
    /// For more details, see ``tokenCount``.
    ///   - groundingMetadata: Output only. Grounding metadata for the candidate. (behavior varies
    /// by backend). For more details, see ``groundingMetadata``.
    ///   - avgLogprobs: Output only. Average log probability score of the candidate. (behavior
    /// varies by backend). For more details, see ``avgLogprobs``.
    ///   - logprobsResult: Output only. Log-likelihood scores for the response tokens and top
