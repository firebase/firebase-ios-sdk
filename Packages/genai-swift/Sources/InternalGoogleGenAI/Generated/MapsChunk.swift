...
public extension GenAITypes {
  ...
  struct MapsChunk: Codable, Sendable, Equatable, Hashable {
    ...
    /// ``uri``.
    ///   - title: Title of the place. (behavior varies by backend). For more details, see
    /// ``title``.
    ///   - text: Text description of the place answer. (behavior varies by backend). For more
    /// details, see ``text``.
    ///   - placeId: The ID of the place, in `places/{place_id}` format. A user can use this
    /// (behavior varies by backend). For more details, see ``placeId``.
