...
public extension GenAITypes {
  ...
  struct Segment: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - partIndex: The index of a Part object within its parent Content object. (behavior varies
    /// by backend). For more details, see ``partIndex``.
    ///   - startIndex: Start index in the given Part, measured in bytes. Offset from the start of
    /// (behavior varies by backend). For more details, see ``startIndex``.
    ///   - endIndex: End index in the given Part, measured in bytes. Offset from the start of
