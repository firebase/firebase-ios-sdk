...
public extension GenAITypes {
  ...
  /// the information.
  struct WebChunk: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - uri: Output only. URI reference of the chunk. (behavior varies by backend). For more
    /// details, see ``uri``.
    ///   - title: Output only. Title of the chunk. (behavior varies by backend). For more details,
    /// see ``title``.
