...
public extension GenAITypes {
  ...
  struct GenerateContentRequest: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - model: Required. The name of the `Model` to use for generating the completion. (Gemini
    /// Developer API only). For more details, see ``model``.
    ///   - systemInstruction: Optional. Developer set [system (behavior varies by backend). For
    /// more details, see ``systemInstruction``.
    ///   - contents: Required. The content of the current conversation with the model. (behavior
    /// varies by backend). For more details, see ``contents``.
    ///   - tools: Optional. A list of `Tools` the `Model` may use to generate the next response.
    /// (behavior varies by backend). For more details, see ``tools``.
    ///   - toolConfig: Optional. Tool configuration for any `Tool` specified in the request. Refer
    /// to the (behavior varies by backend). For more details, see ``toolConfig``.
    ///   - safetySettings: Optional. A list of unique `SafetySetting` instances for blocking unsafe
    /// content. (behavior varies by backend). For more details, see ``safetySettings``.
    ///   - generationConfig: Optional. Configuration options for model generation and outputs.
    /// (behavior varies by backend). For more details, see ``generationConfig``.
    ///   - cachedContent: Optional. The name of the content (behavior varies by backend). For more
    /// details, see ``cachedContent``.
    ///   - serviceTier: Optional. The service tier of the request. (Gemini Developer API only). For
    /// more details, see ``serviceTier``.
    ///   - store: Optional. Configures the logging behavior for a given request. If set, it takes
