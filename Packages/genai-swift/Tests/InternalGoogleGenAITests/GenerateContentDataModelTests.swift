...
@Suite struct GenerateContentDataModelTests {
  @Test func testDecodeUnarySuccessMockResponses() throws {
    ...
    let mockResponsesDir = packageDir
      .appendingPathComponent("utilities/vertexai-sdk-test-data/mock-responses")

    #expect(
      FileManager.default.fileExists(atPath: mockResponsesDir.path),
      "Mock responses directory not found. Did the repo clone fail?"
    )
      ...
      for fileURL in files {
        ...
        do {
          if filename.contains("token") {
            _ = try decoder.decode(GenAITypes.CountTokensResponse.self, from: data)
          } else {
            _ = try decoder.decode(GenAITypes.GenerateContentResponse.self, from: data)
          }
          decodeCount += 1
        } catch {
      ...
      for fileURL in files {
        ...
        for (lineIndex, line) in lines.enumerated() {
          ...
          guard trimmed.hasPrefix("data:") else {
            Issue
              .record("Line \(lineIndex + 1) in \(subDir)/\(filename) does not start with 'data:'")
            continue
          }
          ...
          guard let jsonData = jsonString.data(using: .utf8) else {
            Issue
              .record(
                "Failed to convert line \(lineIndex + 1) in \(subDir)/\(filename) to UTF8 data"
              )
            continue
          }

          do {
            _ = try decoder.decode(GenAITypes.GenerateContentResponse.self, from: jsonData)
            chunkCount += 1
          } catch {
            Issue
              .record(
