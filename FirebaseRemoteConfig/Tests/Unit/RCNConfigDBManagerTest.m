...
static NSString *const RCNDatabaseName = @"rcn_test";
        ...
        [_DBManager
            loadMainWithBundleIdentifier:bundleIdentifier
                       completionHandler:^(
                         ...
                         NSString *fullyQualifiedNamespace = [NSString
                             stringWithFormat:@"%@:%@", namespace_p, kFIRDefaultAppName];
    ...
    RCNDBCompletion insertCompletion = ^void(BOOL success, NSDictionary *result) {
      ...
      if (count == 100) {
        ...
        NSString *stringValue =
            [[NSString alloc] initWithData:result[@"key100"] encoding:NSUTF8StringEncoding];
...
- (void)testWriteAndLoadExperiments {
  ...
  NSDictionary *payload3 =
      @{ @"experiment_ID" : @"35667",
         @"experiment_activate_name" : @"activate_game" };
    ...
    RCNDBCompletion writeMetadataCompletion = ^(BOOL success, NSDictionary *result) {
      ...
      RCNDBCompletion readCompletion = ^(BOOL success, NSDictionary *experimentResults) {
        ...
        XCTAssertEqualWithAccuracy(
            -11, [experimentResults[@RCNExperimentTableKeyMetadata][@"last_known_start_time"]
                     doubleValue],
            1.0);
