...
- (void)loadMainWithBundleIdentifier:(NSString *)bundleIdentifier
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(justSmallDelay * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
- (void)loadPersonalizationWithCompletionHandler:(RCNDBLoadCompletion)handler {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(justOtherSmallDelay * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
- (void)testCheckAndWaitForInitialDatabaseLoad {
  ...
  // Check that no one of first three calls of `-checkAndWaitForInitialDatabaseLoad` do not produce timeout error <begin>
  XCTestExpectation *expectation1 = [self expectationWithDescription:@"1st `checkAndWaitForInitialDatabaseLoad` return without timeout"];
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    [configContent checkAndWaitForInitialDatabaseLoad];
    [expectation1 fulfill];
  });
  XCTestExpectation *expectation2 = [self expectationWithDescription:@"2nd `checkAndWaitForInitialDatabaseLoad` return without timeout"];
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    [configContent checkAndWaitForInitialDatabaseLoad];
  ...
  // This make us more that both `-load...` methods synched with `-checkAndWaitForInitialDatabaseLoad`.
  XCTAssertTrue(mockDBManager.isLoadMainCompleted);
  XCTAssertTrue(mockDBManager.isLoadPersonalizationCompleted);

  // Check that no one of first three calls of `-checkAndWaitForInitialDatabaseLoad` do not produce timeout error <end>.
