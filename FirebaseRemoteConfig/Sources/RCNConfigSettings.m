...
#import "third_party/firebase/ios/Releases/GoogleUtilities/Environment/third_party/GULAppEnvironmentUtil.h"
...
// TODO(mandard) Update logic for app extensions as required.
...
- (NSDictionary *)loadConfigFromMetadataTable {
  ...
  if (metadata) {
    // TODO(mandard) Remove (all metadata in general) once ready to
...
- (void)updateInternalContentWithResponse:(NSDictionary *)response {
  ...
  // TODO(mandard): Investigate why we need to insert into the database on a key by key basis
...
- (NSString *)nextRequestWithUserProperties:(NSDictionary *)userProperties {
  ...
  // TODO(mandard) confirm _deviceAuthID , _secretToken and deviceDataVersionInfo are not needed
