...
#import "third_party/firebase/ios/Releases/FirebaseInstanceID/Library/FIRInstanceID+Private.h"
#import "third_party/firebase/ios/Releases/FirebaseInstanceID/Library/FIRInstanceIDCheckinPreferences.h"
#import "third_party/firebase/ios/Releases/FirebaseCore/Library/FIROptions.h"
#import "third_party/firebase/ios/Releases/FirebaseCore/Library/Private/FIRErrorCode.h"
  ...
  [instanceID
      fetchCheckinInfoWithHandler:^(FIRInstanceIDCheckinPreferences *preferences, NSError *error) {
        ...
        dispatch_async(fetchCheckinInfoWithHandlerSelf->_lockQueue, ^{
          ...
          if (error) {
            FIRLogError(kFIRLoggerRemoteConfig, @"I-RCN000023",
                        @"Failed to fetch checkin info: %@.", error);
          } else {
            ...
            if (strongSelf->_settings.deviceAuthID.length &&
                strongSelf->_settings.secretToken.length) {
  ...
  RCNConfigFetcherCompletion fetcherCompletion = ^(NSData *data, NSURLResponse *response,
    ...
    dispatch_async(fetcherCompletionSelf->_lockQueue, ^{
        ...
        if (statusCode != kRCNFetchResponseHTTPStatusCodeOK) {
          ...
          if (statusCode == kRCNFetchResponseHTTPStatusTooManyRequests ||
            ...
            if ([strongSelf->_settings shouldThrottle]) {
              // Must set lastFetchStatus before FailReason.
              // TODO(mandard) See if we can make a method in RCNConfigSettings to set these.
              strongSelf->_settings.lastFetchStatus = FIRRemoteConfigFetchStatusThrottled;
              strongSelf->_settings.lastFetchError = FIRRemoteConfigErrorThrottled;
      ...
      if (fetchedConfig) {
        ...
        [_content updateConfigContentWithResponse:fetchedConfig forNamespace:_FIRNamespace];
        // Update experiments.
      ...
      _settings.lastETag = ((NSHTTPURLResponse *)response).allHeaderFields[kETagHeaderName];

      [_settings updateMetadataWithFetchSuccessStatus:YES];
      return [strongSelf reportCompletionOnHandler:completionHandler
                                        withStatus:FIRRemoteConfigFetchStatusSuccess
                                         withError:nil];
    });
