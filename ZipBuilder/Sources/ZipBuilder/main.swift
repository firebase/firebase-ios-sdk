...
/// Describes an object that can check if a file exists in the filesystem. Used to allow for better
...
struct LaunchArgs {
  ...
  init(userDefaults defaults: UserDefaults = UserDefaults.standard,
    ...
    guard let templatePath = defaults.string(forKey: Key.templateDir.rawValue) else {
      LaunchArgs.exitWithUsageAndLog("Missing \(Key.templateDir) key for the folder containing " +
        "all required files to build frameworks.")
    }
  ...
  private static func exitWithUsageAndLog(_ errorText: String) -> Never {
    ...
    fatalError("Invalid arguments. See above for specific error and usage instructions.")
  }
}
...
do {
  let location = try builder.build()
