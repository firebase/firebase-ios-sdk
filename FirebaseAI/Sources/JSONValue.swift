...
// MARK: - SharedDataModels Mappings

import SharedDataModels

extension JSONValue {
  package func toShared() -> SharedDataModels.JSONValue {
    switch self {
    case .null:
      return .null
    case let .number(value):
    ...
    case let .bool(value):
      return .bool(value)
    case let .object(obj):
      return .object(obj.toShared())
    case let .array(arr):
      return .array(arr.map { $0.toShared() })
    }
  }

  package init(fromShared val: SharedDataModels.JSONValue) {
    switch val {
    case .null:
      self = .null
    case let .number(value):
    ...
    case let .bool(value):
      self = .bool(value)
    case let .object(obj):
      self = .object(JSONObject(fromShared: obj))
    case let .array(arr):
      self = .array(arr.map { JSONValue(fromShared: $0) })
    }
  }
}

extension JSONObject {
  package func toShared() -> SharedDataModels.JSONObject {
    return mapValues { $0.toShared() }
  }

  package init(fromShared obj: SharedDataModels.JSONObject) {
    self = obj.mapValues { JSONValue(fromShared: $0) }
  }
}
