...
public final class FirebaseAI: Sendable {
  ...
  ///     [available model names ](https://firebase.google.com/docs/ai-logic/models#available-model-names) for a
  ...
  // TODO: Remove the `#if compiler(>=6.2.3)` when Xcode 26.2 is the minimum supported version.
    ...
    /// **[Public Preview]** Creates a ``GeminiModel`` for the specified model name.
    ///
    /// > Warning: This API is a public preview and may be subject to change.
    ///
    /// - Parameters:
    ///   - name: The name of the model to use; see [available model names ](https://firebase.google.com/docs/ai-logic/models#available-model-names)
    ...
    ///   - requestOptions: Configuration parameters for sending requests to the backend.
    public func geminiModel(name: String, safetySettings: [SafetySetting]? = nil,
                            requestOptions: RequestOptions = RequestOptions())
      -> GeminiModel {
      return GeminiModel(
        modelName: name,
    ...
    ///
    /// - Parameters:
    ///   - model: The name of the model to use; see [available model names ](https://firebase.google.com/docs/ai-logic/models#available-model-names)
    ///     for a list of supported model names.
    ...
    public func generativeModelSession(model: any LanguageModelProvider,
      ...
      return GenerativeModelSession(
        model: model._languageModel(firebaseAI: self),
        tools: tools,
  ...
  ///
  /// - Note: Refer to [the Firebase docs on the Live API](https://firebase.google.com/docs/ai-logic/live-api#models-that-support-capability) for
  /// guidance on choosing an appropriate model for your use case.
  ///
  ...
  private func agentPlatformModelResourceName(modelName: String, location: String) -> String {
    guard !location.isEmpty && location
      ...
      https://firebase.google.com/docs/ai-logic/locations?api=vertex#available-locations \
