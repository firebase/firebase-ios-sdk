...
/// guide](https://firebase.google.com/docs/ai-logic/server-prompt-templates/get-started).
public final class TemplateGenerativeModel: Sendable {
  ...
  ///     include `String`, `Int`, `Double`, `Float`, `Bool`, arrays, and nested dictionaries;
  ...
  public func generateContent(templateID: String,
                              inputs: [String: Any] = [:]) async throws
    -> GenerateContentResponse {
    return try await generateContentWithHistory(
      history: [],
      template: templateID,
      inputs: TemplateInput.inputs(from: inputs)
    )
  }
  ...
  public func generateContentStream(templateID: String, inputs: [String: Any] = [:]) throws
    ...
    return try generateContentStreamWithHistory(
      history: [],
      template: templateID,
      inputs: TemplateInput.inputs(from: inputs)
    )
  ...
  ///   - inputs: A dictionary of variable names and values to substitute into the template; the
  ///     same inputs are sent with every turn of the conversation. Defaults to no inputs.
  ...
  func templateGenerateContentRequest(template: String, inputs: [String: TemplateInput],
    ...
    return TemplateGenerateContentRequest(
      ...
      tools: (internalTools?.isEmpty ?? true) ? nil : internalTools,
  ...
  ///   - inputs: The variables to substitute into the template.
  /// - Returns: The content generated by the model.
  /// - Throws: A ``GenerateContentError`` if the request failed.
  func generateContentWithHistory(history: [ModelContent], template: String,
                                  inputs: [String: TemplateInput]) async throws
    -> GenerateContentResponse {
  ...
  func generateContentStreamWithHistory(history: [ModelContent], template: String,
                                        inputs: [String: TemplateInput]) throws
    -> AsyncThrowingStream<GenerateContentResponse, Error> {
    let request = try templateGenerateContentRequest(
      template: template,
      inputs: inputs,
