...
extension ProtobufValue: Decodable {
  init(from decoder: Decoder) throws {
    ...
    if container.decodeNil() {
      ...
      self = .bool(boolValue)
    } else if let objectValue = try? container.decode(ProtobufStruct.self) {
      self = .object(objectValue)
    } else if let arrayValue = try? container.decode([ProtobufValue].self) {
