...
extension GenerateContentAPI {
  ...
  struct Tool: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - templateFunctions: Optional. A list of user-provided functions for function calling. For
    /// functions whose
    ///   - googleMaps: Optional. Tool to retrieve public maps data for grounding, powered by
    /// Google. (behavior varies by backend). For more details, see ``googleMaps``.
    ///   - functionDeclarations: Optional. A list of `FunctionDeclarations` available to the model
    /// that can be used (behavior varies by backend). For more details, see
    ...
    init(templateFunctions: [TemplateFunction]? = nil,
