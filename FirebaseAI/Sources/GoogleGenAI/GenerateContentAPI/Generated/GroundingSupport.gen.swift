...
extension GenerateContentAPI {
  ...
  /// model's response.
  struct GroundingSupport: Codable, Sendable, Equatable, Hashable {
    ...
    let renderedParts: [Int]?

    /// Creates a new `GroundingSupport`.
    ///
    /// - Parameters:
    ///   - segment: Segment of the content this support belongs to. (behavior varies by backend).
    /// For more details, see ``segment``.
    ///   - groundingChunkIndices: Optional. A list of indices (into 'grounding_chunk' in (behavior
    /// varies by backend). For more details, see ``groundingChunkIndices``.
    ///   - confidenceScores: Optional. Confidence score of the support references. Ranges from 0 to
