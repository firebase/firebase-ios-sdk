...
extension GenerateContentAPI {
  ...
  struct FunctionCallingConfig: Codable, Sendable, Equatable, Hashable {
    ...
    let streamFunctionCallArguments: Bool?

    /// Creates a new `FunctionCallingConfig`.
    ///
    /// - Parameters:
    ///   - mode: Optional. Specifies the mode in which function calling should execute. If
    /// (behavior varies by backend). For more details, see ``mode``.
    ///   - allowedFunctionNames: Optional. A set of function names that, when provided, limits the
    /// functions the model (behavior varies by backend). For more details, see
