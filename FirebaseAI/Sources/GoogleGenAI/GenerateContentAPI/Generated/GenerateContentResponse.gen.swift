...
extension GenerateContentAPI {
  ...
  struct GenerateContentResponse: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - candidates: Candidate responses from the model. (behavior varies by backend). For more
    /// details, see ``candidates``.
    ///   - promptFeedback: Returns the prompt's feedback related to the content filters. (behavior
    /// varies by backend). For more details, see ``promptFeedback``.
    ///   - usageMetadata: Output only. Metadata on the generation requests' token usage. (behavior
    /// varies by backend). For more details, see ``usageMetadata``.
    ///   - modelVersion: Output only. The model version used to generate the response.
    ///   - responseId: Output only. response_id is used to identify each response. (behavior varies
    /// by backend). For more details, see ``responseId``.
    ///   - modelStatus: Output only. The current model status of this model. (Gemini Developer API
