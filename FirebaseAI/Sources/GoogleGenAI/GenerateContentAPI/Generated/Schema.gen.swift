...
extension GenerateContentAPI {
  ...
  final class Schema: Codable, Sendable {
    /// Required. Data type.
    ///
    /// ### Gemini Developer API
    ///
    ...
    /// schema.
    let additionalProperties: ProtobufValue?
    ...
    let defs: [String: Schema]?

    /// Creates a new `Schema`.
    ///
    /// - Parameters:
    ///   - type: Required. Data type. (behavior varies by backend). For more details, see ``type``.
    ///   - format: Optional. The format of the data. Any value is allowed, but most do not trigger
    /// any (behavior varies by backend). For more details, see ``format``.
    ///   - title: Optional. The title of the schema. (behavior varies by backend). For more
    /// details, see ``title``.
    ///   - description: Optional. A brief description of the parameter. This could contain examples
    /// of use. (behavior varies by backend). For more details, see ``description``.
    ///   - nullable: Optional. Indicates if the value may be null. (behavior varies by backend).
    /// For more details, see ``nullable``.
    ///   - `enum`: Optional. Possible values of the element of Type.STRING with enum format.
    /// (behavior varies by backend). For more details, see ```enum```.
    ///   - items: Optional. Schema of the elements of Type.ARRAY. (behavior varies by backend). For
    /// more details, see ``items``.
    ///   - maxItems: Optional. Maximum number of the elements for Type.ARRAY. (behavior varies by
    /// backend). For more details, see ``maxItems``.
    ///   - minItems: Optional. Minimum number of the elements for Type.ARRAY. (behavior varies by
    /// backend). For more details, see ``minItems``.
    ///   - properties: Optional. Properties of Type.OBJECT. (behavior varies by backend). For more
    /// details, see ``properties``.
    ///   - required: Optional. Required properties of Type.OBJECT. (behavior varies by backend).
    /// For more details, see ``required``.
    ///   - minProperties: Optional. Minimum number of the properties for Type.OBJECT. (behavior
    /// varies by backend). For more details, see ``minProperties``.
    ///   - maxProperties: Optional. Maximum number of the properties for Type.OBJECT. (behavior
    /// varies by backend). For more details, see ``maxProperties``.
    ///   - minimum: Optional. SCHEMA FIELDS FOR TYPE INTEGER and NUMBER (behavior varies by
    /// backend). For more details, see ``minimum``.
    ///   - maximum: Optional. Maximum value of the Type.INTEGER and Type.NUMBER (behavior varies by
    /// backend). For more details, see ``maximum``.
    ///   - minLength: Optional. SCHEMA FIELDS FOR TYPE STRING (behavior varies by backend). For
    /// more details, see ``minLength``.
    ///   - maxLength: Optional. Maximum length of the Type.STRING (behavior varies by backend). For
    /// more details, see ``maxLength``.
    ///   - pattern: Optional. Pattern of the Type.STRING to restrict a string to a regular
    /// expression. (behavior varies by backend). For more details, see ``pattern``.
    ///   - example: Optional. Example of the object. Will only populated when the object is the
    /// root. (behavior varies by backend). For more details, see ``example``.
    ///   - anyOf: Optional. The value should be validated against any (one or more) of the
    /// subschemas (behavior varies by backend). For more details, see ``anyOf``.
    ///   - propertyOrdering: Optional. The order of the properties. (behavior varies by backend).
    /// For more details, see ``propertyOrdering``.
    ///   - `default`: Optional. Default value of the field. Per JSON Schema, this field is intended
    /// for (behavior varies by backend). For more details, see ```default```.
    ///   - additionalProperties: Optional. If `type` is `OBJECT`, specifies how to handle
    /// properties not defined in (Gemini Enterprise Agent Platform only). For more details, see
    /// ``additionalProperties``.
    ///   - ref: Optional. Allows referencing another schema definition to use in place of this
...
extension GenerateContentAPI.Schema: Equatable, Hashable {
  static func == (lhs: GenerateContentAPI.Schema, rhs: GenerateContentAPI.Schema) -> Bool {
    return
      lhs.type == rhs.type &&
