...
extension GenerateContentAPI {
  ...
  /// GoogleSearch tool type.
  /// Tool to support Google Search in Model. Powered by Google.
  struct GoogleSearch: Codable, Sendable, Equatable, Hashable {
    ...
    let blockingConfidence: BlockingConfidence?

    /// Creates a new `GoogleSearch`.
    ///
    /// - Parameters:
    ///   - timeRangeFilter: Optional. Filter search results to a specific time range. (Gemini
    /// Developer API only). For more details, see ``timeRangeFilter``.
    ///   - searchTypes: Optional. The set of search types to enable. If not set, web search is
    ///   - excludeDomains: Optional. List of domains to be excluded from the search results.
    /// (Gemini Enterprise Agent Platform only). For more details, see ``excludeDomains``.
    ...
    /// ``blockingConfidence``.
    init(timeRangeFilter: Interval? = nil,
