...
extension GenerateContentAPI {
  ...
  struct TemplateFunction: Codable, Sendable, Equatable, Hashable {
    ...
    /// }
    /// ```
    let inputSchema: [String: ProtobufValue]?
    ...
    let outputSchema: [String: ProtobufValue]?

    /// Creates a new `TemplateFunction`.
    ///
    /// - Parameters:
    ///   - name: Required. The name of the function to call.
    ///   - inputSchema: Optional. Describes the parameters to the function in JSON Schema format.
    /// The schema
