...
extension GenerateContentAPI {
  ...
  /// relevant metrics.
  struct UsageMetadata: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// - Parameters:
    ///   - promptTokenCount: Number of tokens in the prompt. When `cached_content` is set, this is
    /// (behavior varies by backend). For more details, see ``promptTokenCount``.
    ///   - cachedContentTokenCount: Number of tokens in the cached part of the prompt (the cached
    /// content) (behavior varies by backend). For more details, see ``cachedContentTokenCount``.
    ///   - candidatesTokenCount: Total number of tokens across all the generated response
    /// candidates. (behavior varies by backend). For more details, see ``candidatesTokenCount``.
    ///   - toolUsePromptTokenCount: Output only. Number of tokens present in tool-use prompt(s).
    /// (behavior varies by backend). For more details, see ``toolUsePromptTokenCount``.
    ///   - thoughtsTokenCount: Output only. Number of tokens of thoughts for thinking models.
    /// (behavior varies by backend). For more details, see ``thoughtsTokenCount``.
    ///   - totalTokenCount: Total token count for the generation request (prompt + thoughts +
    /// (behavior varies by backend). For more details, see ``totalTokenCount``.
    ///   - promptTokensDetails: Output only. List of modalities that were processed in the request
    /// input. (behavior varies by backend). For more details, see ``promptTokensDetails``.
    ///   - cacheTokensDetails: Output only. List of modalities of the cached content in the request
    /// input. (behavior varies by backend). For more details, see ``cacheTokensDetails``.
    ///   - candidatesTokensDetails: Output only. List of modalities that were returned in the
    /// response. (behavior varies by backend). For more details, see ``candidatesTokensDetails``.
    ///   - toolUsePromptTokensDetails: Output only. List of modalities that were processed for
    /// tool-use request inputs. (behavior varies by backend). For more details, see
    /// ``toolUsePromptTokensDetails``.
    ///   - serviceTier: Output only. Service tier of the request. (Gemini Developer API only). For
