...
extension GenerateContentAPI {
  ...
  struct LogprobsResult: Codable, Sendable, Equatable, Hashable {
    ...
    let chosenCandidates: [LogprobsResultCandidate]?

    /// Creates a new `LogprobsResult`.
    ///
    /// - Parameters:
    ///   - logProbabilitySum: Sum of log probabilities for all tokens. (Gemini Developer API only).
    /// For more details, see ``logProbabilitySum``.
    ///   - topCandidates: Length = total number of decoding steps. (behavior varies by backend).
    ...
    init(logProbabilitySum: Double? = nil,
