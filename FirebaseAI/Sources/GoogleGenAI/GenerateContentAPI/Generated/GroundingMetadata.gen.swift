...
extension GenerateContentAPI {
  ...
  struct GroundingMetadata: Codable, Sendable, Equatable, Hashable {
    ...
    /// source is Google Maps.
    let sourceFlaggingUris: [GroundingMetadataSourceFlaggingUri]?

    /// Creates a new `GroundingMetadata`.
    ///
    /// - Parameters:
    ///   - searchEntryPoint: Optional. Google search entry for the following-up web searches.
    /// (behavior varies by backend). For more details, see ``searchEntryPoint``.
    ///   - groundingChunks: List of supporting references retrieved from specified grounding
    /// source. (behavior varies by backend). For more details, see ``groundingChunks``.
    ///   - groundingSupports: List of grounding support. (behavior varies by backend). For more
    /// details, see ``groundingSupports``.
    ///   - retrievalMetadata: Metadata related to retrieval in the grounding flow. (behavior varies
    /// by backend). For more details, see ``retrievalMetadata``.
    ///   - webSearchQueries: Web search queries for the following-up web search. (behavior varies
    /// by backend). For more details, see ``webSearchQueries``.
    ///   - imageSearchQueries: Image search queries used for grounding. (behavior varies by
    /// backend). For more details, see ``imageSearchQueries``.
    ///   - googleMapsWidgetContextToken: Optional. Resource name of the Google Maps widget context
    /// token that can be used (behavior varies by backend). For more details, see
    /// ``googleMapsWidgetContextToken``.
    ///   - retrievalQueries: Optional. The queries that were executed by the retrieval tools.
    /// (Gemini Enterprise Agent Platform only). For more details, see ``retrievalQueries``.
    ...
    /// ``sourceFlaggingUris``.
    init(searchEntryPoint: SearchEntryPoint? = nil,
