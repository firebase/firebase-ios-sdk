...
extension GenerateContentAPI {
  ...
  struct VideoResponseFormat: Codable, Sendable, Equatable, Hashable {
    ...
    ///
    /// Optional. The duration for the video output.
    let duration: ProtobufDuration?

    /// Creates a new `VideoResponseFormat`.
    ///
    /// - Parameters:
    ///   - delivery: Optional. Delivery mode for the generated content. (Gemini Enterprise Agent
    /// Platform only). For more details, see ``delivery``.
    ///   - gcsUri: Optional. The Google Cloud Storage URI to store the video output. Required for
    /// Vertex (Gemini Enterprise Agent Platform only). For more details, see ``gcsUri``.
    ///   - aspectRatio: The aspect ratio for the video output. (Gemini Enterprise Agent Platform
