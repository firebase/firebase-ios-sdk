...
- **`Backend.swift`**: Used to configure the backend API (Gemini Enterprise API or Gemini Developer API).
...
Contains extensions to Firebase AI types and external model types.

### Sources/Extensions/Public/

- **`FirebaseAI+GeminiLanguageModel.swift`**: Adds the `geminiLanguageModel(name:)` factory method to `FirebaseAI` for integrating with Apple's Foundation Models framework.

### Sources/Extensions/Internal/

- **`FirebaseInfo+Headers.swift`**: Provides request header generation for Firebase AI and Gemini Language Model requests.
- **`GeminiLanguageModel+Firebase.swift`**: Extends `GeminiLanguageModel` with an initializer configured using a `FirebaseAI` instance.
