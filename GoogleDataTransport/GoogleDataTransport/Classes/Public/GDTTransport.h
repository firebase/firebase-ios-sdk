 ...
 *
 * @param mappingID The mapping identifier used by the backend to map the extension to a proto.
...
/** Copies and sends an internal telemetry event. Events sent using this API are lower in priority, and
 * sometimes won't be sent on their own.
 *
 * @note This will convert the event's extension proto to data and release the original event.
 *
 * @param event The event to send.
 */
- (void)sendTelemetryEvent:(GDTEvent *)event;

/** Copies and sends an SDK service data event. Events send using this API are higher in priority, and
