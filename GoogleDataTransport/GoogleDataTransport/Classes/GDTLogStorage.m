...
@implementation GDTLogStorage

+ (instancetype)sharedInstance {
  static GDTLogStorage *sharedStorage;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    sharedStorage = [[GDTLogStorage alloc] init];
  });
...
- (instancetype)init {
  self = [super init];
  if (self) {
    _storageQueue = dispatch_queue_create("com.google.GDTLogStorage", DISPATCH_QUEUE_SERIAL);
...
- (void)storeLog:(GDTLogEvent *)log {
  ...
  __block GDTLogEvent *shortLivedLog = log;
  __weak GDTLogEvent *weakShortLivedLog = log;
  log = nil;
  ...
  dispatch_async(_storageQueue, ^{
    ...
    id<GDTLogPrioritizer> logPrioritizer =
        [GDTRegistrar sharedInstance].logTargetToPrioritizer[@(logTarget)];
    GDTAssert(logPrioritizer, @"There's no prioritizer registered for the given logTarget.");

    // Write the extension bytes to disk, get a filename.
    GDTAssert(shortLivedLog.extensionBytes, @"The log should have been serialized to bytes");
    ...
    if (shortLivedLog.qosTier == GDTLogQoSFast) {
    ...
    if (weakShortLivedLog) {
      GDTLogError(GDTMCELogEventWasIllegallyRetained, @"%@",
...
- (NSSet<NSURL *> *)logHashesToFiles:(NSSet<NSNumber *> *)logHashes {
  ...
  dispatch_sync(_storageQueue, ^{
    for (NSNumber *hashNumber in logHashes) {
      ...
      GDTAssert(logURL, @"A log file URL couldn't be found for the given hash");
...
- (void)removeLog:(NSNumber *)logHash logTarget:(NSNumber *)logTarget {
  ...
  GDTAssert(error == nil, @"There was an error removing a logFile: %@", error);
  ...
  GDTAssert(logHashes, @"There wasn't a logSet for this logTarget.");
...
- (NSURL *)saveLogProtoToDisk:(NSData *)logTransportBytes logHash:(NSUInteger)logHash {
  ...
  if (!writingSuccess) {
    GDTLogError(GDTMCEFileWriteError, @"A log file could not be written: %@", logFilePath);
  }
 ...
 */
- (void)addLogToTrackingCollections:(GDTLogEvent *)log logFile:(NSURL *)logFile {
...
static NSString *const kGDTLogHashToLogFileKey = @"logHashToLogFileKey";

/** The NSKeyedCoder key for the logTargetToLogHashSet property. */
static NSString *const kGDTLogTargetToLogHashSetKey = @"logTargetToLogHashSetKey";
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  ...
  GDTLogStorage *sharedInstance = [self.class sharedInstance];
  dispatch_sync(sharedInstance.storageQueue, ^{
    ...
    sharedInstance->_logHashToLogFile = [aDecoder decodeObjectOfClass:NSMutableDictionaryClass
                                                               forKey:kGDTLogHashToLogFileKey];
    sharedInstance->_logTargetToLogHashSet =
        [aDecoder decodeObjectOfClass:NSMutableDictionaryClass forKey:kGDTLogTargetToLogHashSetKey];
  });
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  GDTLogStorage *sharedInstance = [self.class sharedInstance];
  dispatch_sync(sharedInstance.storageQueue, ^{
    [aCoder encodeObject:sharedInstance->_logHashToLogFile forKey:kGDTLogHashToLogFileKey];
    [aCoder encodeObject:sharedInstance->_logTargetToLogHashSet
                  forKey:kGDTLogTargetToLogHashSetKey];
  });
}
