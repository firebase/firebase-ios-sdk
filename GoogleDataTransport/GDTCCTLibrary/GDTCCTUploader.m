  ...
  dispatch_once(&CSHOnceToken, ^{
    ...
    dispatch_once(&INTOnceToken, ^{
      ...
      const char URL[77] = {
          ...
          p1[22], p2[22], p1[23], p2[23], p1[24], p2[24], p1[25], p2[25], p1[26], p2[26], p1[27],
          p2[27], p1[28], p2[28], p1[29], p2[29], p1[30], p2[30], p1[31], p2[31], p1[32], p2[32],
          p1[33], p2[33], p1[34], p2[34], p1[35], p2[35], p1[36], p2[36], p1[37], p2[37], '\0'};
...
- (BOOL)readyToUploadTarget:(GDTCORTarget)target conditions:(GDTCORUploadConditions)conditions {
    if (self.isCurrentlyUploading) {
      GDTCORLogDebug(@"%@",
                     @"CCT: Wait until previous upload finishes. The current version supports "
