...
if [ $SDK == "FirebaseFirestore" ]; then
  ...
  # Github Actions.
  scripts/third_party/travis/retry.sh scripts/pod_lib_lint.rb "Firebase${SDK}".podspec --platforms="$(tr '[:upper:]' '[:lower:]'<<<${platform})" --test-specs=unit
fi
