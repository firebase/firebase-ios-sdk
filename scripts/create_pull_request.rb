...
def generate_pr_for_target_changes(repo_root:, target_path:)
  system("cd #{repo_root}")
  system("git checkout -b #{BASE_BRANCH}\n #{PR_BODY}\n")
  if `git diff #{TARGET_PATH}`==""
    puts "The file has no changes."
    return
  end
  system("git commit -m \"#{COMMIT_COMMENT}\"\n git push -u origin target_diff_detected")
  client = Octokit::Client.new(access_token: ACCESS_TOKEN)
  client.create_pull_request("firebase/firebase-ios-sdk", "master", "target_diff_detected", PR_TITLE, PR_BODY)
end
