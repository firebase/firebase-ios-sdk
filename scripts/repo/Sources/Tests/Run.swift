...
extension Tests {
  struct Run: ParsableCommand {
    nonisolated(unsafe) static var configuration = CommandConfiguration(
      ...
      usage: """
          tests run [--overwrite] [--secrets <file_path>] [--xcode <version_or_path>] [--platforms <platforms> ...] [--sdk <sdk>]

          tests run --xcode Xcode_16.4.0 --platforms iOS macOS --sdk AI
          tests run --xcode "/Applications/Xcode_15.0.0.app" --platforms tvOS --sdk Storage
          tests run --overwrite --secrets ./scripts/secrets/AI.json --sdk AI
        """,
      discussion: """
    ...
    @Option(parsing: .upToNextOption, help: "Platforms to run rests on.")
    ...
    static let log: Logger = Logger(label: "Tests::Run")
    ...
    /// 
    /// The `xcode` argument can be either a full path to the application, or just the application name.
    ...
    private func findXcodeVersions() throws -> [URL] {
      let applicationDirs = FileManager.default.urls(
        ...
        // file manager lists application dirs that CAN exist, so we should check if they actually do exist before trying to get their contents
        let exists = FileManager.default.fileExists(atPath: url.path())
        if !exists {
            log.debug("Application directory doesn't exists, so we're skipping it.", metadata: ["directory": "\(url.path())"])
        }
        return exists
      }

      log.debug("Searching application directories for Xcode installations.", metadata: ["directories": "\(applicationDirs)"])
      ...
      let xcodes = allApplications.filter { file in
        ...
        if !isXcode {
            log.debug("Application isn't an Xcode installation, so we're skipping it.", metadata: ["application": "\(file.lastPathComponent)"])
        }
        return isXcode
      }
