...
package struct Candidate: Codable, Sendable, Equatable, Hashable {
  ...
  ///
  /// - Parameters:
  ///   - index: Output only. Index of the candidate in the list of response candidates. (behavior
  /// varies by backend). For more details, see ``index``.
  ///   - content: Output only. Generated content returned from the model. (behavior varies by
  /// backend). For more details, see ``content``.
  ///   - finishReason: Optional. Output only. The reason why the model stopped generating tokens.
  /// (behavior varies by backend). For more details, see ``finishReason``.
  ///   - finishMessage: Optional. Output only. Details the reason why the model stopped generating
  /// tokens. (behavior varies by backend). For more details, see ``finishMessage``.
  ///   - safetyRatings: List of ratings for the safety of a response candidate. (behavior varies by
  /// backend). For more details, see ``safetyRatings``.
  ///   - citationMetadata: Output only. Citation information for model-generated candidate.
  /// (behavior varies by backend). For more details, see ``citationMetadata``.
  ///   - tokenCount: Output only. Token count for this candidate. (Gemini Developer API only). For
  /// more details, see ``tokenCount``.
  ///   - groundingMetadata: Output only. Grounding metadata for the candidate. (behavior varies by
  /// backend). For more details, see ``groundingMetadata``.
  ///   - avgLogprobs: Output only. Average log probability score of the candidate. (behavior varies
  /// by backend). For more details, see ``avgLogprobs``.
  ///   - logprobsResult: Output only. Log-likelihood scores for the response tokens and top tokens
  /// (behavior varies by backend). For more details, see ``logprobsResult``.
  ///   - urlContextMetadata: Output only. Metadata related to url context retrieval tool. (behavior
  /// varies by backend). For more details, see ``urlContextMetadata``.
