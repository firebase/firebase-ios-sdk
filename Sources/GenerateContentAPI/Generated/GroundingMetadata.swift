...
package struct GroundingMetadata: Codable, Sendable, Equatable, Hashable {
  ...
  /// (behavior varies by backend). For more details, see ``groundingChunks``.
  ...
  ///   - webSearchQueries: Web search queries for the following-up web search. (behavior varies by
  /// backend). For more details, see ``webSearchQueries``.
  ///   - imageSearchQueries: Image search queries used for grounding. (behavior varies by backend).
  /// For more details, see ``imageSearchQueries``.
  ...
  ///   - retrievalQueries: Optional. The queries that were executed by the retrieval tools. (Gemini
  /// Enterprise Agent Platform only). For more details, see ``retrievalQueries``.
  ///   - sourceFlaggingUris: Optional. Output only. A list of URIs that can be used to flag a place
  /// or review for (Gemini Enterprise Agent Platform only). For more details, see
  /// ``sourceFlaggingUris``.
