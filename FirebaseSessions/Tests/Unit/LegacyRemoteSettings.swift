...
// of Firebase v12.9.0.
// It is used exclusively for round-trip persistence testing to ensure
...
final class LegacyRemoteSettings: SettingsProvider, Sendable {
  ...
  private let cache: UnfairLock<LegacySettingsCacheClient>

  private var sessionsCache: [String: Any] {
    cache.withLock { cache in
      cache.cacheContent[LegacyRemoteSettings.flagSessionsCache] as? [String: Any] ?? [:]
    }
  }

  init(appInfo: ApplicationInfoProtocol,
       downloader: SettingsDownloadClient,
       cache: LegacySettingsCacheClient = LegacySettingsCache()) {
  ...
  var sessionsEnabled: Bool? {
    return sessionsCache[LegacyRemoteSettings.flagSessionsEnabled] as? Bool
  }

  var samplingRate: Double? {
    return sessionsCache[LegacyRemoteSettings.flagSamplingRate] as? Double
  }

  var sessionTimeout: TimeInterval? {
    return sessionsCache[LegacyRemoteSettings.flagSessionTimeout] as? Double
  }
