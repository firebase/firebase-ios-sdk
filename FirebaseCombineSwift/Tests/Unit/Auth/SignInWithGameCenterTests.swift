...
class SignInWithGameCenterTests: XCTestCase {
    ...
    fileprivate static let timestamp: UInt64 = 12345678
    ...
    class MockBackendRPCIssuer: NSObject, FIRAuthBackendRPCIssuer {
        ...
        func asyncPostToURL(with requestConfiguration: FIRAuthRequestConfiguration, url URL: URL, body: Data?, contentType: String, completionHandler handler: @escaping FIRAuthBackendRPCIssuerCompletionHandler) {
            self.requestURL = URL
            if let body = body {
                requestData = body
    ...
    func testRequestResponseEncoding() {
        ...
        FIRAuthBackend.signIn(withGameCenter: request)
            .sink { completion in
                ...
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.playerIDKey] as? String, Self.playerID)
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.publicKeyURLKey] as? String, Self.publicKeyURL)
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.signatureKey] as? String, Self.signature)
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.saltKey] as? String, Self.salt)
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.timestampKey] as? UInt64, Self.timestamp)
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.accessTokenKey] as? String, Self.accessToken)
                XCTAssertEqual(RPCIssuer.decodedRequest?[Self.displayNameKey] as? String, Self.displayName)
